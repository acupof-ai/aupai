...
contract fb set for a hand-curated pattern list:
  ...
  2. Each DROP sample matches the pattern it is paired with (index-aligned), so the
     {pattern -> sample} map is an honest per-rule witness. A witness MAY also match sibling
     rules -- cross-matching is allowed and not asserted against; the only claim is that its
     own rule catches it. Truly redundant rule pairs (a witness that survives deleting its
     own rule because a sibling identical rule also catches it) are excluded EXHAUSTIVELY by
     the deletion mutation in item 4, not by the per-witness assertion.
  3. A set of normal-clean KEEP strings is dropped by NO rule (the false-kill guard).
  4. Mutation: deleting any one pattern from PATTERNS must leak that pattern's witness
     (drops() goes false for it). Verified exhaustively out-of-band (all 171 deletions); a
     pattern whose witness is covered only by a sibling is a redundant rule and is reported.
"""
