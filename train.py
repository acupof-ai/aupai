...
class Cfg:
    ...
    # CED, CAUSAL ENCODER-DECODER, is the ONLY architecture (user order 2026-09-23: flat deleted,
    # no on/off flag; spec = runs/prereg.jsonl#v41_ced_0923 + docs/standards/v41_pivot.md:9-15).
    # The bottom `ced_enc_layers` layers are the encoder; every decoder layer projects its global
    # KV entries C^l and compression weights Z^l from the encoder's FINAL hidden state H_split by
    # that layer's OWN W_KV/W_Z (Eq.1, per-layer UNSHARED). Layer-local SWA is unchanged and
    # attention stays causal everywhere.
    ced_enc_layers = 0   # encoder/decoder split; 0 = L//2. The gate line passes --ced_enc_layers 6 explicitly
...
def main():
    ...
    parser.add_argument("--ced_enc_layers", type=int, default=None,
                        help="CED: how many bottom layers are the encoder; 0 = L//2, the gate "
                             "line passes 6 (default: Cfg.ced_enc_layers=0)")
