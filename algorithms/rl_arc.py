#!/usr/bin/env python3
"""GRPO RL on ARC-Easy (non-arithmetic reasoning).
...
Usage: torchrun --nproc_per_node=N algorithms/rl_arc.py [--steps 200] [--group_size 8]
"""
...
ARC_PATH = os.path.join(ROOT, "data", "arc_easy.jsonl")
...
def main():
    ...
    if is_main:
        print(
            f"RL-ARC: {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M params, "
    ...
    if is_main:
        print(f"ARC: {len(questions)} questions", flush=True)
