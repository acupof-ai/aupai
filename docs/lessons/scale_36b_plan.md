...
**t19 落地（2026-08-31，44 实测，e1 格式逐字节复刻，真实答案渲染）：**

| 档 | 每轮 p50 | 每轮 p90 | 每轮 p99 | 4096 装几轮（p90） |
|---|---|---|---|---|
| code（Thought+代码块=调用，沙箱执行=tool） | 994 | 1729 | 2608 | **1.9 轮** |
| math（GSM8K 表达式=调用，结果=tool） | 18 | 21 | 22 | 188 轮 |

**条件式结论：4096 对 agentic code 在当前数据形状下是硬约束（p90 1.9 轮 < 2）。约束来自调用回合里的长 Thought 散文（p90 调用回合 1729 token），不来自上下文本身——math 侧同样的 4096 装得下 188 轮，tool 回合也便宜（400 条真实执行输出 p50=35 字符、p90=327）。先试短调用回合的渲染（nanochat 式：`12/60 = ` 几个 token 就停），那不动架构、不动冻结配方；不够再谈加长。** 没测的一半：模型在短调用回合下还能不能推理——那是能力问题，不是上下文问题，两个不混。

zh-en 提示成本不对称：同等含义的 agentic system prompt，中文 33 token / 英文 36 token——**每字符更贵 ≠ 每条指令更贵**（中文每字符含义更密）。35:65 的 token 成本不对称，已落 facts。

tool 输出长度是已知上界（只测 400 条，打印大列表的代码会推高），30B 语料筛选时反过来用：配对 impl+test 挖掘时按 tool 输出长度过滤。

**chat 现在是 mix 的 1.18%，而本表 `wiki + chat` 合起来只有 1%。对 agentic 目标这一格是在缩。** 多轮结构和「停下等外部输入」与代码、数学一样要靠预训练量堆出来——但 0.2B 不是零:它存在的意义是让 ChatML 和两回合 tool 渲染不是从零学(SFT 包安装格式也才 192M tokens,0.2B 预训练已经同量级)。**裁定(2026-08-31,fb):wiki_chat 保持 0.2B。** 提权重的触发条件写死:**8B 里程碑上,tool 回合 span 的 per-role loss 持平而其他角色在动 → 在 anneal 段提,不在那之前提。** 依赖长度的能力是 t19/上下文问题,不是权重问题。
...
## 1b. 30B 运行配方（2026-08-31 定稿）

阶梯的冻结配方**不整体继承**——它冻结是为了六个点逐字可比，而 30B 不是阶梯点，没有东西要和它可比。逐项裁定：

| 项 | 30B 取值 | 依据 |
|---|---|---|
| `seq` | **4096，不动** | 语料只有 4-5% 的文档超过 4096（`code` 只有 0.4%：p50=152、p90=445）。`doc_mask=True` 下，把 seq 提到 8192 只是在一行里塞更多互相独立的文档，**不产生更长的依赖**，而三层完整因果注意力的 FLOPs 要翻四倍。推理端可超出：配对 ΔNLL 探针实测零交叉 ~6.2K，8K 处 +0.104 nat。 |
| `warmup` | **300 步**（阶梯是 20） | 见下方推导。 |
| `world` / `batch` / `accum` | 7 / 16 / 2，不动 | 有效 batch 224，与阶梯一致；改它会同时改梯度噪声。 |
| 其余冻结项 | 不动 | 架构与 `vocab` 无理由改。 |

**warmup 的推导，以及它什么时候失效——这一段是这张表里唯一会腐烂的数：**

warmup 罩的是「优化器自己的统计量还不可信」的那一段，而那段长度由 β2 和动量 ramp 决定，**是绝对步数，和 run 有多长无关**。`train.py` 里三个写死的绝对步数：

```
warmup                20 步
Muon momentum ramp   150 步   0.85 + 0.10*min(1, step/150)
embed Adam 1/(1-β2)  200 步   embed_betas = (0.8, 0.995)
```

下界 `max(200, 150) = 200`，取 **300** 留余量。**300 = max(embed β2 视界, Muon ramp) + 余量，不是一个通用常数**：谁改了 `embed_betas` 或那个 `step/150`，300 就要跟着重算。

按比例算是错的推理方式：1% 在 30B 上恰好给出 327（和 300 只差 27），**但在 300B 上会给出 3270，那不对应任何东西**。两个数接近掩盖了推理方式的不同，而差别要到下一个尺度才显形。

阶梯的 warmup 20 是对的——那六个点最短的只有 218 步，20 是它的 9.2%。**同一个值在 30B 的 32,697 步上是 0.061%，等于没有 warmup。** 冻结的目的（可比性）在 30B 上消失了，冻结本身却还在；这是「派生约束在它的源条件消失后仍然生效」，`ladder_config_frozen` 的判据已改成问 `LADDER` 清单本身，而不是问文件名前缀——否则 `mix_scale_30b.json` 这个最自然的名字会让 30B **静默地**继承 warmup 20。

**一个阶梯的附带事实：0.2b 全程 218 步，而 embed Adam 的二阶矩视界是 200 步。** 那个点几乎整段跑在优化器统计量不可信的区间里。六个点配方一致所以拟合没坏，但**小点测到的东西里，优化器瞬态占了相当份额**——这影响对 E/β 的解释，不影响可比性。

**训练时长：~16 小时**（六个点的实测墙钟外推；吞吐随规模上升并收敛到 ~524K tok/s 于 7 卡：0.2B/425s、0.8B/1577s、1.6B/3117s、3.24B/6180s）。
...
- **`harness run dedup --domains <a,b,c>` → `datagen/dedup_corpus.py`**（de 第 4 步）。**全局跨源 pass**（dedup 后 clean、score 前）。输出是一个 dedup 清单加一份统计，都带 `dedup_fp`（**计划中的产物，尚不存在**；路径在实现时定，此处不写路径 —— `doc_commands_exist` 会把文档里的路径当成应当存在的文件）。CPU-only、按域续跑。
