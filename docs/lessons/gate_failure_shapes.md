...
## 61. 判据的取值范围没覆盖到它要区分的那件事(de 起草,2026-09-02,五个实例,与 §9/§54 同族)
...
代价不对称:1 和 2 让一个真缺陷上卡(93.78 GiB OOM),3 让一个会改变数值的重构过关,4 制造了一次 OOM,5 只是虚惊。但五个的修法是同一个——**把判据从「词出现过」收紧到「结构成立」**:1/2 改成走 AST(iterator 必须 unparse 成 `range(train.Cfg.accum)`),3 要求输入偏离单位元,4 改读 `memory.used` 求和,5 加时间下界。
