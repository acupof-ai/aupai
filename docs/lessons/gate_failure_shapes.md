...
5 incidents (5 model, all 2026-09-08), ~2h each, 10h. `manual:` — no single check fits, because the defect is the absence of a case rather than a wrong case. The nearest machine-checkable halves are per-instance: `deletion_candidates.py::_selftest` asserts that its citation glob subsumes every single-ledger predicate, `domain_bpb.py::_selftest` asserts a known answer on an input longer than `max_ctx`, and `harness.py::_selftest_sideways_move_names_what_it_discarded` asserts that a ledger MAIN wrote is absent from the loss report.
...
That all five landed on one day, in one session's work, is the finding. §275 named the shape; §276 is the shape inside the fix for §275; §277 is the same shape in an unrelated file, found by a peer while §276 was being fixed; §278 is the shape in the INSTRUMENT built to measure §277, and it manufactured a defect that had not occurred; §279 is the shape in a FIXTURE, written the same evening by someone who had just read all four. Naming a shape does not immunise the next thing you write against it, and the later instances say the exposure grows rather than shrinks once you are hunting the shape — each probe you write to chase it is another hand-drawn population, and so is each world you build to test the probe.
