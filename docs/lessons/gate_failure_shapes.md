...
- §287: a pre-registered criterion that fires on a known-answer NEGATIVE world, because the control arm was silently a replicate of the arm it is compared against. Three arms: N1/N2 differ only by seed so F=|N1-N2| is the floor; R shares N1's seed 1337 and was registered as carrying "the reweight and nothing else". The premise was never checked against build_mix. train.py:2791-2810 builds the MAIN phase FIRST from d["weight"], and the two mixes are byte-identical outside _comment and nine `anneal` values, so idx, ph and the single Cfg.seed-seeded randperm all match -- N1 and R consume the same rows in the same order for 6,866 of 7,629 steps and the reweight cannot act until then. Confirmed in the logs: loss 6.615/6.616 at step 10, IDENTICAL 5.683 and 5.587 at 20 and 30, then 5.220/5.215 and 4.811/4.817. So |R-N1| at the read point is drift plus reweight and the criterion cannot separate them; drift alone reads +0.001/-0.001/+0.011/+0.016 at steps 500-2000 with the reweight inactive. F does not bound it -- N1 and N2 are two runs as well as two seeds, and two samples give a range, not an upper bound on a third. Fix costs nothing: D=|R-N1| at step 6500, the last read before the anneal, is the same-seed drift on these arms, and a verdict needs |R-N1| to exceed BOTH F and D. Limitations stated: D is the 20-batch estimator against the final read's 100-batch one so it bounds scale and is NOT subtractable (§286, one entry later, same experiment), and one pair is one sample. The residual is not narrow: a sqrt extrapolation gives 0.031 (under F) but back-predicts +0.008 and +0.011 where the actuals are +0.001 and -0.001, while a linear fit that tracks all three later points gives 0.113 (over F) -- the extrapolation is not decidable from four points and quoting either number would be R6's failure inside R2's entry.
...
7 incidents (2 infra, 5 model), ~4h each, 28h. `check_cache_readers_set_vocab_id` (registered CHECKS entry) enforces vocab identity on cache readers; `train.py:1472` raises if `VOCAB_ID` is unset. Partial: covers vocab, not all producer identity.
...
Cannot see: whether the identity a checkpoint carries is the identity it ran with (§44, §189, §210); whether a stamped provenance field describes the artifact it is stamped on (§284).
