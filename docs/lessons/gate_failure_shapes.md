...
## 127. 同 run 三份 checkpoint 不是一类:按大小判重会把 .pt、.ep1、.step500 当同一种(b0 当事人,1e 提,2026-09-03,R9 实例)

同一个 run 的三份 checkpoint:.pt 与 .ep1 的权重 sha **逐位相同**(同一权重的两次保存),.step500 差最后一步 optimizer 状态。按大小判重会把三份当同一种——**大小是存储属性,不是内容身份;权重 sha 相同的是重复,optimizer 状态不同的是不同 checkpoint。** 删除候选按大小广播,会把"该留的 step500"和"可删的 .ep1"放进同一个删除类。

规则:**checkpoint 判重按内容(权重 sha + optimizer 状态指纹),不按大小或命名;删除候选广播前,先按内容分类,同一类里才谈删哪个。** 判断法:把候选的权重 sha 并排——相同的是重复(留一),不同的是不同 checkpoint(各自有理由),大小一列不参与判决。
