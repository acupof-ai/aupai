...
## 54. 压缩一个还在变的文档,压缩产物会变成唯一入口(de 观察,44 当事人,2026-09-02,与 §16 同族)

压缩进行中的内容时,读者只看压缩产物——上游的后续修正必须重新穿过压缩才能到达读者,而压缩者已经转去做别的事。

实例:48 个 shape 压缩成 AGENTS.md 的 10 条规则(44-18)。压缩当晚 §49/§50 还没落地,R8 的 §refs 打了剑号「pending de's landing」。之后 de 连写 §49-§53,表和剑号一起停在 48:R8 引用的两个 shape 实际落在别的规则下(§49→R1、§50→R6),R8 自己的措辞「name IO and memory, not metric class」被 de-27 的测量推翻——而表是读者的唯一入口。剑号是补丁不是解:它把「上游在变」写进了产物,但产物仍然过期。
