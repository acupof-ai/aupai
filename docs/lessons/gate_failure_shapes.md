...
**方向是它活下来的原因。** 预测 t+2 本该更难,但位置 t 的输入窗口里**已经含有 token t+1**,错位把答案漏给了模型,loss 不升反降。一个把数字推差的 bug 立刻显眼(曲线坏了);一个把数字推好的 bug 读起来像"模型训得不错"。**数字只会朝一个方向错时,那个方向如果是"更好",它就没有天敌。**
...
同一个双重移位 bug,在随机初始化的 2 层模型上只值 **0.0139 nat(0.3%)**;在训练过的生产 checkpoint 上值 **2.28 倍**。随机模型近乎均匀分布,预测 t+1 和 t+2 一样无望,错位无利可图 —— **泄漏只有在模型学到了值得抄的局部结构之后才付息。**
...
错误指标下 1e-4 胜出,正确评分下 3e-5 胜出 —— **bug 翻转的是选点决策,不是报表上的一个数字**。scan 的全部意义就是比较,而比较的方向被系统性地拧了:预测 t+2 时上下文里已有 t+1,泄漏让所有点的 loss 一起变低,但低的幅度随学习率不同,相对次序就变了。
