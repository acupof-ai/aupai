...
错误指标下 1e-4 胜出,正确评分下 3e-5 胜出 —— **bug 翻转的是选点决策,不是报表上的一个数字**。scan 的全部意义就是比较,而比较的方向被拧了:训练与评测共用同一个错对齐(§83),模型学的是 p(x[t+2]|x[0..t]),数值自洽但测的是错误条件。排序为什么翻转,没有泄漏可解释 —— 未训练模型上双移位 ≈ ln(V),上下文里没有答案可漏(§79);实测事实就是错误目标上的 loss 地貌对 LR 的排序不同。
