...
status: open
source: derived from docs/lessons/gate_failure_incidents.md (82 model-project incidents) and docs/lessons/infra_incidents.md (87 pod/infra incidents); 33 closed incidents removed 2026-09-04 (169 = 82 + 87); 33/33 confirmed machine-gated (list below)
---
...
74 incidents (34 infra, 40 model), ~4h each, 296h. `manual:` no check verifies that a criterion expresses the property asked; `--selftest` requires every CHECKS entry to carry `broken()`, but a selftest that passes on a broken world is invisible to the contract.
...
### R2-b Population narrower than the property (16 incidents)
...
Cannot see: whether the test's inputs, environment, or scale match the property's (§26, §29, §34, §35, §40, §48, §65, §72, §121, §146, §151, §169, §180, §201).
