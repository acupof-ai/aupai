...
- §267: an assertion whose search pattern is an occurrence of the string it searches for matches its own line, so it cannot go red when its subject is deleted. `grep -q '<literal>' "$0"` checking that a call site exists passes with the call site gone; three cases in one file, two caught only by a mutation sweep. Fix by requiring count ≥ 2 (the assertion's own line plus the subject) or by searching for a string the assertion does not contain — and note the escaped-vs-bare `$` trap that makes counting fail on a correct file. A source-level assertion's population always contains itself.
