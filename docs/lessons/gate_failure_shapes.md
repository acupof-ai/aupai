...
- §267: an assertion whose search pattern is an occurrence of the string it searches for matches its own line, so it cannot go red when its subject is deleted. `grep -q '<literal>' "$0"` checking that a call site exists passes with the call site gone; three cases in one file, two caught by a mutation sweep and the third by the reviewer after the "fix" reintroduced it. Require count ≥ 2 (the assertion's own line plus the subject); "search for a string the assertion does not contain" reads like the cleaner fix and is nearly unachievable — any pattern specific enough to identify the subject gets quoted in the assertion, and the shell escaping that would break the self-match is what breaks the count. A source-level assertion's population always contains itself.
