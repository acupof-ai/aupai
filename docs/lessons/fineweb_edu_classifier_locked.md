...
- **可复算（2026-09-23 重验）**：从 HuggingFaceFW/fineweb-edu-classifier 重新下载（model.safetensors sha256[:32] `27fbfe65ce71c3fc50df7b71246c83fe`，437,955,572 B，与 0916 同权重），CPU 27s，编辑后的 `scripts/score_fineweb_edu_locked.py` 复现全部聚合量，并写出逐 doc 的 `runs/fwe_locked400_perdoc.jsonl`（400 行 {id,score,junk,rewrite}）。两个 AUC 不需模型即可从该文件重算：`roc_auc_score(score,~junk)`=0.422、`roc_auc_score(score,~rewrite)`=0.5563。
