...
> **更正(e1 2026-09-03Z,6e 批;预注册原文一字未改,错的理由不删只标)。**
> 上表 RL gap 行括号里的 **`pass@8⊇pass@1 同题相关`** 描述的不是本仓的实现:
> `eval/math_hard.py:177` 是 `lv[2] += int(any(oks[1:]))`,**greedy 不在那 k 次抽样里**
> (该行注释自己写着 "greedy is not one of the k draws")。**所以 pass@8 不是 pass@1 的超集,
> gap 可以为负**——一道 greedy 做对而 8 次采样全错的题,对 gap 贡献 −1。
>
> **SE 的量级结论不变,理由更正为两向不一致。** 配对差 SE = √(b+c)/n,b/c 是两个方向的
> 不一致题数:嵌套(b=40,c=0)→ 0.61pt;非嵌套(b=40,c=10)→ 0.69pt;(b=40,c=40)→ 0.87pt。
> **三种都远低于 15pt 闸口(15pt 需要 b−c = 155 道题@1036),所以"SE ~1-2pt"成立、三态判决不变。**
>
> **为什么标而不是只在心里知道:那个 `⊇` 会让下一个人以为 gap 有单侧性**,据此写一道
> "gap < 0 说明测量坏了"的守卫,而 gap < 0 在这个实现下是合法观测。**结论对不保证论证对,
> 后来人继承的是论证**(§18);而一道对合法观测报警的守卫,正是 §153 的第二个方向
> ——它的失败是静默的,读者看不出被拒的是坏数据还是好数据。
>
> **本条在数落地之前写下(pass@k 当时跑到 192/1032)**,所以它是关于代码的陈述,
> 不是关于某个数的解释。
