...
### 7.5 语料更名后跨阶段 per-role 熄灯;进度/保留/组成三轨分立(2026-09-01,b0 提问 / 44 裁决)

**触发**:16B(step17500)对 8B 自有 mix 基线(step8500)的 per-role 被 head guard 整指标拒绝。pod 指纹实证四套语料(en_c4/en_c4_stage2、math_owm/math_owm_stage2 各异),5/7 域在 stage 2 更名重建。**拒绝正确**:2/7 角色会跨不同 held-out 文本比较,正是 3.24B 失败类。guard 先于 §7.1 触发也是正确顺序——head 不配比权重变更更根本。

**结构事实**:stage 2 更名了七个域中的五个,**任何 stage-2 checkpoint(16B、22B、30B)都无法在自己的头上与 stage-1 基线做 per-role 进度比较**。这不是 16B 的临时状态,是 stage 2 的永久状态。

**裁决——三轨分立,现在写明,不在 30B 时才发现**:

1. **进度轨(per-role 只在阶段内)**:per-role 进度读数只在同阶段 checkpoint 之间进行(16B→24B→30B,同头、同语料字节、同 mix)。跨阶段 per-role 进度**熄灯**。30B 产出里没有跨阶段 per-role 进度读数——如此而已,不是缺口。阶段内比较不触发 §7.1(同 mix 无权重可变)也不触发 §7.2(同语料字节),就是同一分布上的规模进度,显著性用 σ̂ 阈值。
2. **保留轨(新增,标注 OOD)**:stage-2 里程碑在 **stage-1 头**上打分,列名 `retention (OOD)`,回答的问题是"stage-2 模型是否灾难性遗忘旧分布",不是"stage-2 配比是否更好"。它**不进阈值机械,不叫进度,不进 per-role 判决表**。前置条件:按 §7.2 第 2 条,stage-1 holdout 哈希对 stage-2 语料重探,命中 = 污染 = 拒绝该角色的保留读数(stage-2 重建可能与 stage-1 语料有重叠文本,不验就打是裸奔)。
...
**§7.1 的状态是休眠,不是通过**。同 mix 下权重闸无可闸;读数须写 `dormant: no mix change between these checkpoints`,**永不写 "passed"**。且 §7.1 从未在真实数据上跑过(首个真实用例前它必须先有合成开火测试——一个没跑过的闸等于没测过的闸,P6;若 stage 3 改权重,那是它首次真实开火)。

**守卫改逐角色,不整指标拒绝**:现 guard 在 head 集不一致时拒绝整个 metric,把 2 个干净角色(cot 等未更名域)的信号一起扔了。改为逐角色判定:头匹配的角色判,不匹配的拒绝,拒绝消息逐角色列出。干净角色的阶段内/保留读数照出。

**预注册规则(持久,逐边界重新应用)**:*per-role 比较仅当两模型在字节同一的 held-out 头上打分时有效。训练语料跨阶段变更时,跨阶段 per-role 进度默认熄灯;阶段内 per-role 是进度指标;跨阶段保留是单独标注的 OOD 指标,且 holdout 哈希须对新语料重探。*
