...
- **few-shot arm(e1/de,已落地,a7f2f74):** 纯续写 3-demo 8.13% vs 同聚合 shuffle 2.40%(z=8.42,正确抽取规则下);8-demo 5.69%,示例数无效应(配对符号检验 p=0.073,不报方向)。**R3 判定后被收回(2026-09-01 修正,见 §10):8.13% 低于最强常数基线(恒答"2"=9.78%)——高于 shuffle 是真的,高于随机是假的;答对的 40 道全在小整数区间,与 gold 众数重合,是分布投机不是算术。**两个仪器教训:(a) 续写无终止符,43.5% 的 3-demo 生成(216/497)会另起一道 `题目：` 自编自解——全缓冲 last-box 取到自编题答案(25/497),first-box 误杀本轮中间结果(45/497),正确规则 = 截断在首个新 `题目：`、取本轮内最后一个 box(40/497);(b) 8-demo 格式率 33.5%(3-demo 80.5%,旧口径含答后 box)——示例给多了反而压制答案格式,两臂测的不是同一件事,8-demo 准确率不与 3-demo 并排读。
...
4. **最强常数基线(2026-09-01 修正新增,§10):** 评测集 gold 分布的众数频率——恒答众数值能得的分。math_test_500(491 行)只有 169 个不同 gold,恒答"2"=9.78%。**判定线是 max(shuffle 对照, 常数基线),不是 shuffle  alone。** shuffle 防抽取器乱匹配,常数防分布投机,两者防的不是同一种失败,并排报。

## 4. 报告顺序(提取率优先)

1. 成形/提取率,每档每域,先于一切通过率。
2. **拒报线:提取率 <20% 的档,不报通过率**——报"提取地板,通过率不可读"。
3. 通过率与对照同行:严格、format-free、shuffle 对照、**常数基线**、鹦鹉下限,五个数一行。
...
- **OOE-a:** 数学 format-free 任一档 > max(shuffle 对照, 常数基线) + 2δ → 30B token 上有可读算术能力;500M 有了正基线。结论从"无仪器"变"仪器存在、弱"。**(2026-09-01 修正:原版只比 shuffle,已被常数基线推翻——8.13% vs 9.78%,OOE-a 不触发,见 §10。)**
...
- **R1 确实不会:** 最佳档(成形最高)提取率 ≥20% 且 format-free 通过率 ≤ max(shuffle+2pt, 常数基线) 且严格 ≤3% 且 通过率−鹦鹉下限 ≤2pt。模型产出答案形状的文本,不比随机、常数投机或复述好。配上 free_running FR/TF ≤0.05(已测),判决:**200M/30B 上生成式推理缺席,不是格式阻塞。** **(2026-09-01 修正:floor 从 shuffle 改为 max(shuffle, 常数),见 §10。step24000 落此判:8.13% < 9.78%。)**
...
- **R3 会一点:** 提取率 ≥20% 且 format-free > max(shuffle+2pt, 常数基线) + 2δ 但 <12.6pt → 弱可读信号;报 Δ 和剂量-反应,不下能力判决。**(2026-09-01 修正:floor 同上。)**
...
- **预登记的回答(2026-09-01 按已落地数更新):** 200M 尺度读得出信号的仪器族是 **log-prob 判别**(two-way 偏好、cloze)——math_v2_like ~95%、L0' 易扰动层 100%(27c8a7f 基线),都饱和,在 500M 上只能当护栏。生成类仪器在 200M 的读数**低于常数基线**(8.13% < 9.78%,§10):仪器管线存在(抽取/对照/剂量-反应都能跑),但 200M 的能力未被证明,不是"弱信号"。500M 读数:(a) 退化率先导(每档,免费,前段唯一有量程的快仪器,readout_design §8);(b) 生成通过率判决,**判定线 = max(shuffle, 常数基线)+ 2δ**,每 3B;(c) 判别类仪器作退化护栏,不作进步读数。
...
## 10. 2026-09-01 修正:最强常数基线(de 发现,fb 裁定)

**触发事实(de 实测):** `math_test_500` 的 491 行只有 169 个不同 gold 值,72.9% 的行答案与别的行重复;恒答"2"得 9.78%。模型 3-demo 8.13%、8-demo 5.69%、p324 0.2%——**三个 L1 点全部低于常数基线**。模型输出分布(1: 9.4%、5: 7.5%、2: 5.1%;答对的 40 道里 3/4/2 占 26)与 gold 众数区间重合:答对是分布投机,不是算术。

**修正内容:**
1. 判定线从"shuffle + 2pt"改为"**max(shuffle 对照, 常数基线) + 2δ**"。shuffle 防抽取器乱匹配,常数防分布投机——两种失败不同,两个对照并排报,不替换。
2. **R3"会一点"收回,step24000 改判 R1"确实不会"**(机制:小整数分布匹配)。OOE-a 不触发(原版只比 shuffle,已被推翻)。
3. **通则(fb 裁定,推广到所有评测):任何选择题式或短答案式评测,判定线必须是最强常数策略,不是随机。** 答案分布偏斜是自然语料常态,随机基线系统性低估地板。代码侧常数基线 ~0%(常数程序过不了任意测试),数学侧必须算。
4. 没被推翻的:ChatML 语料缺席(语料扫描)、0.3% vs 94.4% `def ` 存在性计数、退化率 74–99%——都不是正确性评分,不受影响。

**时序说明:** 本修正在 de 的 gold 分布实测之后,但改的是判定线不是结论——R1-R4 框架(测前写死)不变,变的是框架里 floor 的定义。fb 2026-09-01:"预期写在数字之前,所以这次改的是判定线,不是结论。"
