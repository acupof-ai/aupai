...
de-13 cursor (the first resume is this window; the second re-reads without it) → de-23 train half → de-20 → d57273f (domain_loss reads the checkpoint's mix) → e1-23 required flags → e1-22 dispatch and continuation prompts → e1-16 → tilerl-14/15 → b0-8 model split → b0-14 (step line logs every optimizer group's lr; the embedding group runs at 0.1 while the line prints Muon's 1e-2) → de-29 (run-end save passes step; today ckpt_p200m_4b_0902.pt claims 0.87B unread tokens and only .ep1 is a valid resume target). Then `prove_resume`, `harness check` 0 FAIL, `pod_push --all`, relaunch with `--resume ckpt_p500m_20b_0902.milestone_stopwindow1_step3000.pt` through `supervise_run.sh`, first 50 steps read against the pre-stop loss.
