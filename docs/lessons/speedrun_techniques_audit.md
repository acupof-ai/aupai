...
| Drop gradient clipping | NC LOG: 0.25-2.0 all within noise, all-reduce costs ~2%; MN none | `clip=1.0` plus one CPU sync per step | 1 A/B, throughput read |
...
| Sharded optimizer with reduce_scatter | MN #6 (15.2->13.1 min), #24, #36; NC MuonAdamW | DDP all-reduces full grads and every rank runs the full Newton-Schulz | measure first: nccl is 2.3% of the step at 200M/4 cards (eff.step_class_breakdown_p200m_4card) |
...
| Gradient clipping 1.0 | on | off in both | ~2% step time plus a CPU sync; no measured benefit at either reference |
