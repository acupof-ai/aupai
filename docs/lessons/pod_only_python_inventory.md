...
**本文分两个阶段，写的时候是第一个，现在是第二个。**
§0-§4 是分类（写于分类阶段，那时一个字节都没删）；**§3.5 是 2026-09-02 执行的
16 个删除**，过了三关。读 §5「不能做的事」时注意它写在删除之前——
下面每条都标了现在还成不成立。

fb 的硬边界二：先知道每个文件是什么，再动它。**顺序是这个，两个阶段都在。**
...
**这一节写在 §3.5 的删除之前，逐条标注现状：**

- **不删任何 §2 里的文件。** 它们的字节在 git 里找不到对应物，**那是唯一的一份**。
  → **仍然成立**，§3.5 一个都没碰。
- **不移动它们。** 移动会改变 `pod_drift` 的读数，而发车正在用那个读数。
  → **仍然成立。**
- **~~56 个"可删"的也不在这次动~~。** → **已作废两次**：数字先被
  §1 的 collation 更正从 56 改成 20，然后 §3.5 从 20 里删了 16（余 4 保留，
  理由见 §3.5 末段）。**一条"不做某事"的约束，在事情做完之后仍然留在页面上，
  读起来和活约束一模一样**——这就是为什么每条都要标现状而不是删掉。

## 6. 下一步（发车后）

1. ~~56 个字节在历史里的：过第二关和第三关，再删。~~ **已完成**（§3.5：20 个候选，
   删 16，留 4）。
