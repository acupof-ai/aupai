...
2. **零漂移不足以让 pod 的仓库扫描 FAIL 可归因**——漂移检查的视野是 manifest 子集,pod 上有 233 个未登记文件在视野外。初版"漂移状态下 FAIL 不可归因"只对了一个方向;反过来不成立。
3. **权威 = pod,且需要两个零**:pod_drift=0(已登记文件一致)**且**未登记文件为 0(或仓库扫描类检查把自己的视野限制到已登记文件)。修法方向(报 fb 裁定,不自己动):让漂移检查的视野等于被检文件的视野——要么 pod_drift 扩到全树,要么树扫描类检查只扫 manifest。
