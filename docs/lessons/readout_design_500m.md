...
- **滚动档不带 opt/step(tilerl 2026-09-01 实测 ckpt_shape500_probe.pt:keys 无 opt,step=None)——matched-token 比较轴只能用里程碑档。** 若 L0' 基线 ≤60%(快主读数成立,§4),分辨率要求**里程碑间隔 ≤1B**(不是 save_every ≤1B):1B 网格自动包含 16B/22B 匹配点(整数 B 网格),20 个里程碑 ≈ 37 GB,盘允许。1.25B 与 1B 不互相整除;44 建议 1B 网格——它是 1.25B 对齐的超集,16B/22B 都在格点上,fb 裁定。
