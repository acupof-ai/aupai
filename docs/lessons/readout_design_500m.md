...
**⚠️ 2026-09-02 再修订(fb 裁定,8 卡 + 分段撤销):运行中读数归零,全部推到收尾。** 分段已撤(三个独立理由,见 gate_failure_shapes.md shape 7/23 实例);8 卡建议下运行期间无通道卡、段间窗口也不存在——每 1B 里程碑 sweep 运行中一次都跑不了。
- **运行中可见的是日志**:loss/gnorm/吞吐/峰值显存。生成质量读数全部推到收尾,用 de 的兜底:里程碑 .stepN 照留,跑完用任意一张卡补评。
- **分辨率不降(还是每 1B 一档),降的是时效**:从「跑着知道」变「跑完知道」。这是两个不同的损失,只有时效是真的。
- **钉点成本随卡数分叉(tilerl)**:7 卡钉点免费——200M 与 7 卡 500M 都是 917,504 tok/step,钉点同为 step 17,500 零偏差;8 卡要钉 15,313 + 插值机制。
- **用户若选 7 卡,本条整段作废**,恢复运行中每 1B 评测(通道卡回来)。
...
- **分配:** 快护栏每档都跑;生成 sweep 每 3 档跑一次(~每 3B token)。20B 跑程 ≈ 7 次生成 + 20 次快读 ≈ 12h,全在收尾(运行中为零——8 卡无通道卡),三天预算内留足复测。**节奏全在 token 空间,与吞吐无关**:2026-09-02 现场实测 12K tok/s/gpu × 8 = 96K tok/s(p500m_20b_0902.log steps 90–200;532K 是 200M 形状的吞吐误写进 500M 计划,e1-10 修 milestone_profile.md)——跑程 58h、里程碑每 2.9h 落地一个,但档数不变:每 1B 一个里程碑(~20 个)、生成每 3B 一档(~6-7 个点,即 fb 2026-09-02 转达的 6 点)。
