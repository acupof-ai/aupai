...
**⚠️ 2026-09-02 修订(fb 裁定 8 卡发车):tok/step 从 917,504 变为 1,048,576,本文的步对齐算术作废,比较轴改用 token 对齐。** world=8:32×1×4096×8 = **1,048,576 tok/step**;200M 对照(917,504)不变。后果:
- **步对齐没了**:200M 的 16b = step 17500 = 16.056B token;500M 在 world=8 下 16.056B = step 15,312.5(非整数)。比较轴改为 **token 数对齐**——用 checkpoint 的 token 戳配对,最近点 + 插值,不再钉同一步。
- **§3 网格重算**:1B = 953.67 步;save_every 954 → 间隔 1.0004B;16B 最近格点 step 15,259(15.999B)。
- **launch_30b.sh:81 当前 `--save_every 500`(每 524M),与 1B 网格不一致——发车前必须定,钉点丢了补不回来。**
...
- **存档要求(fb 2026-09-01 终裁,tilerl 执行):** **每 1B 一个里程碑 + 精确钉 16B 一个**(200M 存活档里唯一落在 20B 量程内的匹配点);**前 5B 每 0.4B 加密**(无 200M 对照段,只要分辨率);**1.25B 网格退役**(只买到一个匹配点,不值得让分辨率让路)。~~步数算术(tilerl 2026-09-01 落地):`save_every = 1e9 ÷ 917,504 = 1089.91 → 1090 步`(网格点间隔 1.00008B;第 16 个网格点 step 17440 = 16.0013B,±1.3M token,0.008%——"1B 网格"在执行层是这个精度,不存在整数步精确落在 16B)。精确钉点 = step 17500:500M 与 200M 同为 917,504 tok/step(b32 world=7 或 b16+accum2 都是这个数),步对齐即 token 精确对齐,200M 的 16b 里程碑就是 step17500——钉同一个步,不是钉"16.000B"。~~ **(2026-09-02 修订:以上按 world=7/917,504 tok/step 计算,8 卡发车后作废,见文首修订块——网格 save_every 954、16B 钉点改 token 对齐。)** **里程碑一个不删,只有滚动档参与回收。**单档 1.84 GB(Muon 单动量 + bf16,tilerl 实测),~20 里程碑 ≈ 37 GB(可用 401 GB)。
...
- **结果(2026-09-01,step24000,卡 0):win_rate 1.0(328/328),等 token 子集 291/291 同为 1.0 → ">80% 饱和"分支命中。** 易扰动层(单算子变异)在 200M 上零量程,留作护栏,与 math_v2_like 同命运;仪器验证通过(100% > 90% bar)。**"L0' 把分辨率缩到 1B"关闭。** 唯一剩余 headroom 路径:难扰动层(从 step24000 冻结的模型生成失败解,§6 冻结纪律)——建成后重测基线,若 200M 对自己的失败解仍 ~100% 则 L0' 整条路关闭,若落入可读区间则升快主读数。
- **难扰动层(2026-09-01 在建,27c8a7f):** `--freeze-hard` 从锚点 checkpoint 采样 K=8 条 T=0.8 解,执行过滤,留首个失败非空解,冻结为 JSONL(seed 42 可复现;冻结文件即后续所有 checkpoint 的共同对手)。预登记分支:**覆盖率 <30%(模型产不出代码样失败解)或 win_rate >90%(仍饱和)→ L0' 整条路关闭;60–90% → 升快主读数,分辨率 3B→1B 成立。**
...
- **分段结构:** A 段(0–16B,无 200M 能力对照)退化率先导 + 绝对阈值;B 段(16B+)能力 matched-token 全仪器。**分辨率现状:退化率(免费,每档)是前段唯一快仪器;L0' 易扰动层已饱和(§4),1B 分辨率的能力半悬置,等难扰动层。** 通过率仍每 3B 确认。
- **200M 基线:** 同一 harness 在 5 个存活 checkpoint(16B/22B/尾部三档)重跑退化率,~30 min。de 的 16B 数和 free_running 的 22B 数提示族不同,不能直接拼曲线。
- **16B 钉点停跑生成类能力评测(fb 2026-09-01 裁定,98 送达):** 退化被处理之前,step17500 不读生成通过率——16B 的测量被解码卡死不是被能力卡死,贪心 99.4% 比任何 pass@1 有信息量。"处理" = 本节退化率基线落地(5 个存活 checkpoint,同一 harness);基线落地前,16B 钉点只有退化率一个读数。
