...
| 污染判据依赖测量规模，非绝对阈值 | recorded | CCI3-HQ + web_hq 对照扫描（facts/contamination.json#cont.cci3_scale_failure） | `--full-doc` 下「任一片命中即整源拒」在 ~1GB 分片上**拒绝包括已训语料在内的一切**：web_hq（0830v1 在训）72/124 片 REJECT、CCI3 84/96，两者 exact 全 0。~50 万篇/片 × 1254 题 ≈ 6e8 配对，每对 1e-5 假阳就成千命中 |
| 污染判据的正确输出是 scale-free 速率 + 已训同规模基线 | recorded | CCI3-HQ vs web_hq 每 GB/每百万篇（facts/contamination.json#cont.cci3_vs_webhq_rates） | 绝对计数在不等大语料间无意义。CCI3-HQ 每 GB=43.3 vs web_hq=134.5（CCI3=web_hq 的 0.32×），exact 全 0 → 按 fb 预定规则接受。web_hq 每 GB 反而比 CCI3 高 3 倍。b0 的旧 FPR 0/1498 只在 269 篇上测，对 500K 篇/分片无约束力（规模边界已写入 cont.fpr.boundary）。扫描器默认应输出速率 + 强制 baseline（已交 b0） |
