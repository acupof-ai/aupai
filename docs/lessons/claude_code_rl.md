...
| 3. Isolation per rollout | a temporary git worktree per rollout plus a process sandbox from an existing open-source tool, chosen by what the host offers: `bwrap`/`nsjail`/`firejail` where present, else `unshare`-less fallback of rlimits + timeout + private tmpdir + `datagen/sandbox_exec` for test execution; network off except the shim; K rollouts in parallel; the same code path reports which level of isolation it got | none | de | K=8 rollouts of one task on the pod (no container available) and on the Mac, diffs and test results collected, isolation level recorded per rollout |
