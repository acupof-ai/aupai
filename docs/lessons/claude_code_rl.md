...
|---|---|---|---|---|
| 1. API adaptation | an Anthropic Messages-compatible shim in front of tileRL's OpenAI server: messages, system, tools, `tool_use`/`tool_result` blocks, streaming, `stop_reason`; per request it logs request id, prompt and completion token ids, per-token logprobs; Claude Code points at it through `ANTHROPIC_BASE_URL` | none: tileRL's `tiny` model on CPU | tilerl | **met 2026-09-02, on a rewritten condition** — two assertions, because the original ("tiny completes a tool call and a final answer") is unreachable by construction: a randomly initialised 2-layer model emits noise to `max_tokens` and can never emit a well-formed call, so that gate would have stayed red forever. (i) **pipeline** — one `claude -p` writes one record row, `len(logprobs) == len(completion_ids)`, valid `stop_reason`, response header id == row id; live against a served `tiny-agent`. (ii) **semantic** — a scripted engine (same submit/poll/step duck type) drives `tool_use` → `tool_result` → final answer; `tests/test_server.py::test_messages_tool_use_round_trip`. Detail, the captured field table and the 9x token-estimate error: `tileRL/docs/lessons/messages_shim_stage1.md` |
