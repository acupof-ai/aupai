...
**0.2b A/B cancelled — experiment invalid (2026-08-30)**: `attn_every 1` removes all KDA layers, but GatedMLA is NoPE ("KDA handles position", train.py:272). The model has NO position information. Probe result: val 4.765 vs 3.679 (1.086 nat, 21σ) — measures the value of having ANY position information, not KDA vs attention as operators. All paired design work (σ_d, ρ, falsifier, 721 GPU-min costing) is void — the treatment arm is not a coherent model.

**Guard added**: `train.py` HybridLM now refuses `attn_every=1` (zero KDA layers + NoPE = no position information).

**Throughput finding (stands)**: `attn_every 1` is 44% slower (612s vs 425s at 0.2b, same token count). Consistent with quadratic attention over 4096 positions. This is a throughput measurement, not an MDE judgement.

**Reframed question**: The original "KDA vs attention as operators" has no clean single-variable answer — KDA is load-bearing for position. The meaningful comparison is **"KDA+NoPE vs attention+RoPE"** — two coherent architectures, both with position information, different mechanisms (recurrent state vs rotary embedding). This requires: (1) adding RoPE to GatedMLA (code change, not a flag), (2) accepting the 44% throughput cost, (3) re-deriving MDE for the new comparison. Post-ladder experiment, not scheduled.
...
**3.24b comparison — VOID (2026-08-30)**: All options (A, A', B) are void because `attn_every 1` is not a coherent model (NoPE, no position information). The paired design, σ_d, ρ, falsifier — all elaborating statistics of a comparison whose treatment arm doesn't work. The question must be reframed (see "Reframed question" above) before any 3.24b comparison is designed.
...
1. **KDA+NoPE vs attention+RoPE** — reframed question, needs RoPE code change + 44% throughput cost acceptance. Post-ladder, not scheduled.
