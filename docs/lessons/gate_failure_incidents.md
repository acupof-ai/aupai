...
open: no check inspects a monitor's parsing of its watched stream; the checkable slice is a packing delimiter that also appears in the carried payload. The re-armed monitor itself is still ad-hoc — the MONITOR BLIND/HEARTBEAT pattern is recorded in this incident but the script is tracked nowhere, so the pattern lives where no review can reach it.
