...
Evidence: 4c's pod-log reading (launch times 14:49Z and 16:59Z, SIGTERM while tokenizing `math_owm_stage2`, final val 1.823); `train.py:2132` (`seedfp = cache + ".seed"`), `:2172` (`same_seed`), `:2225-2244` (the write order); PR #111 (`--sample_seed`); `scripts/harness.py` `_derive_gate_timeout` and `_wait_for_startup`.
