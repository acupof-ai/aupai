...
Step timings from the run that carried the fix are bimodal, and **the population has to be named or the median means nothing**: two people computed two medians, 734 s and 49.5 s, both correct because they answered different questions. The pooled set below is "every branch and event that ran this step"; the main-only set is a different population and is reported separately.

The pooled set -- every run carrying step 17, any branch, any event, 31 successful attempts (`startedAt -> completedAt`, the runs API):
...
Fourteen land at 33-50 s and seventeen at 730-1031 s, median 734, with nothing between 50 and 730. The **main-only** population is a different set and is reported separately: restricting to runs on `main`, 33 successful attempts give 21 fast at 33-56 s and 12 slow at 728-979 s, median 51, again nothing between 56 and 728. A second measurement of that same population returned median 49.5 with 9 slow, and the fast clusters agree exactly -- n=21, the same values 33-56 -- so the spread is in the slow tail's count (12 against 9), which is a rounding or window difference worth resolving rather than averaging away. Neither median is "the" step duration, and the earlier text printed the pooled one with no population at all, which is a number anyone can recompute correctly and differently.

Within either set the same rule holds, and this is the part that survives the population question: **a number of seconds is not the class.** 736 s appears twice in the pooled set -- as the cancelled attempt 35584847491/a1, and as a completed SUCCESSFUL step, 35591857451/a1 -- and sits 4 s from the successful 740 s. What separates them is `conclusion`, not the duration, so these timings are only readable with their event type attached; classified by seconds alone the two are indistinguishable.

`ci.yml:73`'s `~2-4 min CPU` estimate for that step sits inside the gap of the pooled set: 240 s is above that set's whole fast cluster and below its whole slow one (a 20x span). The comparison is against the pooled set, and says nothing about the main-only one, whose fast cluster runs to 56 s -- the same estimate is a different factor against a different population.
