...
Step timings from the run that carried the fix are bimodal, and **a whole-set central measure here describes the gap rather than the step**. Over the 31 runs in the table below the mean is 459.9 s, which lands inside the empty 50..730 s interval where no run in the table sits. The median does something different and equally uninformative: it is 734 s, an **observed** value (run 35572684194/a1, just above the boundary), because the slow cluster holds 17 runs against the fast cluster's 14 and the middle order statistic is therefore forced onto a slow point. So the median is not in the gap -- it is pinned to the larger mode and says nothing about the other. What the section states is only what does not move: the empty interval, and the class rule. Cluster centres are stable and are given per cluster -- this table's fast cluster is 33-50 s (n=14, median 43.5) and its slow cluster 730-1031 s (n=17, median 754). **The exact endpoints of the empty interval move with the population, so the interval is the finding and the integers are not**: 50..730 for the 31 rows printed here, and a ~670 s void across the other populations checked. A whole-set rate or centre for this step must state population, window and as-of together, and successive pulls of the *same* labelled window have returned different medians -- the window label alone is not a specification.
