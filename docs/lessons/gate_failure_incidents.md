...
**Instance 1 -- the ordering assertion, `scripts/test_sft_holdout_gate.py`. FIXED 2026-09-19 (genB, PR #564).** The paragraph below is preserved in its original present tense as the diagnosis; the rebuild it names as task #33 / branch b0-40 never happened (b0 left, no such task or branch), and the file sat on main in the broken state until this fix. The line is named "gate is upstream of the ckpt load" and asserts `"nonexistent_ckpt" in o or "No such file" in o`, on the reasoning that reaching the checkpoint error proves the gate let the pack through. That string appears whenever the script reaches the model load at all -- including when the gate sits BELOW it and the checkpoint error fired first, before the gate ran. Green in both orderings, and *greener* in the broken one: move the gate after the load and the message arrives sooner.

Reading `sft_math.py` to write the replacement turned up a second finding the assertion had been hiding: **the name is false as well as the method.** `torch.load(args.resume, ...)` was at `sft_math.py:199`; the holdout gate was at `:265` (stale branch) and `:274` (unstamped branch). The load was UPSTREAM of the gate, not downstream of it, so the property the line claimed to certify did not hold and never did. **(Numbers are the pre-fix tree; the 2026-09-19 fix moved the gate above the load.)** A test that cannot go red also cannot report that the thing it is named for is backwards. The replacement therefore asserts the order that is actually true, read out of the source rather than out of a message, with both indices asserted non-None so a rename goes red instead of vacuously green:
...
## R22. A hardlink is the same file: staging by link then writing in place mutates the source

### §302 (2026-09-13, R22)

**A smoke-cache staging step "copied" vetted corpus files into a throwaway domain with hardlinks, then rewrote them in place; the rewrite truncated the vetted originals to zero bytes. No command failed.**

98 was ordered a CPU-only pretokenize smoke of the vetted textbook dir (fb order, moving the pipeline test ahead of the 200M-token gate). train.py's `_domain_seqs` only globs shard-named files (`SHARD_RE = _\\d{3,}\\.jsonl$`) and reads the `content` key, while 3b's vetted dir holds `claude-gen-A.jsonl` / `claude-gen-B.jsonl` with a `text` key. To produce a train-shaped smoke domain the operator staged `data/corpus/textbooks_vetted_smoke/tb_vetted_000{0,1}.jsonl` with `ln -f` (hardlinks, "zero bytes"), then ran a converter opening each staged path with Python `open(path, "w")`. A hardlink shares the inode; `"w"` truncates the inode to length 0 before writing. Both vetted files became 0 bytes — the smoke-side links and the vetted dir showed link count 2 on the same two inodes (86488687, 86488702) at 0 bytes. The vet stats and vet_failures survived; the raw corpus `textbooks_claude_v41/` survived, so 3b's re-vet reconstructs the pair. No training mix read the vetted dir (r3 stage 1 uses other domains), so the run was untouched; cost was one re-vet, not retraining.

The defect is the "zero-byte copy" intuition: a hardlink is not a copy, and link count > 1 does not protect data — it means two names for one file. The same class already exists elsewhere in this tree (assemble_keep_p1 hardlinks 685 shards BY DESIGN and never mutates them), which is why hardlinks read as safe: they are safe for read-only assembly and fatal the moment a writer opens one name in truncating mode. Nothing along the path is loud — the link succeeds, the write succeeds, and the only witness is `ls -li` showing the shared inode before the write.

Rule: when staging a corpus-derived domain that will be mutated (renamed into shard convention, key-converted, reformatted), always `cp`, or write to a temp file and `os.replace`; never `ln`/hardlink into a path anything will open for writing. Verify distinct inodes (`ls -li` source vs stage) before the first write. The smoke rerun after 3b's re-vet uses copy + temp+rename only.

Cost: two vetted files truncated; one 3b re-vet consumed. Training unaffected. Deletion of the two smoke links and the smoke dir recorded in runs/deletion_0914.txt.
Evidence: inode audit (`find data/corpus -samefile`, 2026-09-13 16:57Z): 86488687 shared only by vetted claude-gen-A.jsonl and smoke tb_vetted_0000.jsonl, 86488702 likewise; train.py:118 SHARD_RE, :1585 `_jsonl_content` ["content"]; runs/deletion_0914.txt; raw corpus intact (4,780 rows at incident time).
open: no check distinguishes a read-only hardlink assembly (keep_p1, intended) from a hardlink into a directory a build step opens for writing; a future staging tool should refuse to link a corpus inode into a domain it is about to mutate.

### §303 (2026-09-21, R13)

**A deletion the guard skipped was invisible in exactly the direction that mattered. The guard that caught the wrong one refused every commit repo-wide; the check's window covers one ref, so a write to any other ref, from the same cause, in the same minute, is not a shape it can see. The freeze was loud and the silent half was worse.**

`de`'s `merge_main.sh de-review-623` wrote its CAS `3785d78c` on the local `main` ref and its push was refused non-fast-forward, because `origin` had advanced when #623 merged. The local ref was left 8 behind and 2 ahead, and `merge_main.sh`'s own recovery text -- "Recover by RE-RUNNING the merge" -- is unsatisfiable in that state: its guard sits after the fetch and before the merge, so a re-run reports the same DIVERGED. The operator cleared it with `git update-ref refs/heads/main refs/remotes/origin/main` (`3785d78c -> 99cdb7e0`).

`main_advances_by_ancestry` correctly reported that as a sideways move. The FAIL is deterministic and repo-wide: the pre-commit hook runs `harness.py check` with no staged-path filter, so it refused every commit in every worktree -- including the friction row documenting it and the `harness.py` fix clearing it. The entry sat at `main@{2}` in a 60-entry window, so it would not have self-expired for roughly 58 more main advances.

Three findings outlive the recovery, and they are the reason this is a shape rather than an incident.

The first is that **the loss was not one row.** Three of the day's review rows existed only on the branch: the #619 approval, the #619 withdrawal, and the #624 approval. A reflog statistic reporting `review.jsonl +1` was read as "one row was discarded", which would have understated the move; the row sets show the session's entire day of review rows on that branch missing from `main`. Counts cannot separate "another session appended N" from "the recovery restored M", and they cannot see a substitution at equal line count. The set difference in the direction concurrency cannot write to -- lines on the branch and not on `main`, since other sessions only ever append to `main` -- is what settles the question.

The second, and the larger one: **the check reads one ref's reflog.** `check_main_advances_by_ancestry` resolves the common git dir and reads `logs/refs/heads/main` (`harness.py:4221-4226`, `:4232`), by design -- `main` lives in the common dir and every session's `merge_main` writes it there. Nothing widens that to a session branch, and the same cause moved one: in the same minute as the `main` write, the local `de-cardclaim-atomic-0921` ref was reset from its own commit `8f849eff` to `f4a8d86f`, a commit already on `main`. The PR was unaffected because its remote ref and the PR head still held `8f849eff`, so the local ref was the only casualty -- an accident, not a protection. A session branch losing its head commit and a session's review rows disappearing from `main` are the same event observed on two refs; the guard covers `main` because `main` is the tree everyone merges through, and it has no subject on a branch at all.

A third effect is a check that reported a weaker state than the loss deserved, for a reason unrelated to the loss. `review_present` is documented to FAIL a close 30 minutes after it if no review row names it, and it has no FAIL branch: it WARNs, and the promised escalation is written into three user-facing places -- `AGENTS.md:587`, `AGENTS.md:619-620`, and the check's own docstring (`harness.py:11066-11124`, three WARN returns at `:11119`/`:11121`/`:11123` and no FAIL). #623 closed at 09:14Z with its approving row written 08:12Z, so the check reported PASS on the chain as it then was -- and the discarded row, had it mattered, could have produced a WARN at most and never the promised FAIL. Two separate defects met here: a row absent from `main` that a reader would have had to reconstruct by set difference, and a gate whose documented escalation does not exist, so the absence of that escalation carried no information either way. The second is worse than a stale docstring because AGENTS.md is what every session reads at start: someone can order their work around a hard gate that is not there. It is the R17 shape family -- "a contract written only in a comment is a check that cannot fail", and §296 adds why that is worse than no contract: a written contract consumes the moment that would have produced doubt, and it has a counterparty, here every session relying on the promised gate. **Not R12**, whose discriminating feature is a green line anti-correlated with its own name and able to pass by not running; this check is not misnamed and always runs, so R12's test does not reach it. (genA and bc both read it as R17; my first draft said R12 and this is my second correction of the same number.)
...
Step timings from the run that carried the fix are bimodal, and **a central measure here has no subject at all**: three people computed three different medians (734 pooled, 754 slow-cluster, 49.5 then 51 for main-only) and every one was arithmetically correct for the set it was taken from. **Every central measure of the pooled set lands inside the void**, not merely the median: the pooled mean is 459.9 s, and no observation between 56 s and 728 s exists in any window queried. A statistic that falls where no datum falls describes the gap between the modes rather than the step. So what this section states is only the property that does not move with the population or the window: the void, and the class rule. Central measures are reported **within a cluster** if at all -- the fast cluster is 33-56 s and the slow one 728-1031 s, and each has its own centre. Anyone quoting a whole-set rate or centre for this step must state population, window and as-of together.
...
`ci.yml:73`'s `~2-4 min CPU` estimate for that step is not compared here against any median: the pooled set has none worth quoting, and the two candidate baselines for the fast side differ (min 33 s, cluster median 44.5 s), so a factor against "the fast cluster" is a factor against a choice. What is checkable and is stated: 240 s falls inside the void, above the fast cluster's every value and below the slow cluster's every value, which is why an estimate placed there tells a reader nothing about which state the step will be in.

Evidence: `scripts/hooks/pre-commit:3148-3151` (the unconditional `harness.py check`, no staged-path filter), `:367-372` (the 2026-09-08 precedent: b0's `484a9528` sideways move "refused every commit repo-wide until PR #54 recorded the pair"); `scripts/harness.py:4221-4232` (the reflog scope is `logs/refs/heads/main`, resolved from the common git dir -- this SCOPE, and not the prefix whitelist, is what leaves a branch ref unreadable to the check), `:4268` (`_RECORDED`), `:4363` (the exemption skip), `:4390` (the unsigned-WARN prefix whitelist, which names `fetch`/`reset`/`branch` as legitimate commands -- it governs only the unsigned column and does NOT exempt anything from the ancestry test, so it is not why the branch write went unreported), `:4432` (the PASS string, whose `len(lines)` is the reflog window length and not the exemption count); commits `766a32c7` (PR #625, the exemption) and `d8ef1711` (the friction row). The reflog pair for the branch write is `8f849eff -> f4a8d86f` on `de-cardclaim-atomic-0921`, read from the local ref's reflog; `origin/de-cardclaim-atomic-0921` held `8f849eff` throughout, and PR #626's head reads `9963cf63` after the two rounds of review that produced it (`8f849eff` -> `0a5afcb9` -> `9963cf63`).

open: `_RECORDED` carries its anti-recurrence information in comments, and nothing prompts anyone to record a pair when the freeze hits -- the third occurrence will again be a hand-typed `update-ref`, reached by the same unsatisfiable advice. The recovery needs a controlled entry point, and the check needs a subject on refs the cause can move: a session branch losing its head is the same event as `main` losing rows and is currently outside the window. Acceptance criterion for the issue, and both directions were run before writing it: (a) in a fixture whose `main` reflog is untouched and whose branch was reset from its own commit to one already on `main`, `check_main_advances_by_ancestry` returns PASS -- measured, so today's guard is blind to the real shape rather than merely unread; (b) a reading over `logs/refs/heads/*` that FAILs when a branch entry's discarded commit is reachable from no remote branch returns FAIL on that fixture and PASS on the control where the same commit was pushed to `refs/remotes/origin/feat` first. The criterion is therefore executable and discriminating, and the issue asks for it with a `broken()` world built by performing that `reset`, not by describing it.
