...
Step timings from the run that carried the fix are bimodal, and **a median here would be a number with no subject**: three people computed three different ones (734 pooled, 754 slow-cluster, 49.5 then 51 for main-only) and every one of them was arithmetically correct for the set it was taken from. The median of a bimodal set measures the gap, not the data; the main-only median also drifts with the window queried (latest-60 gives 49.5, latest-100 gives 730 -- genA). So what this section states is only the property that does not move: the void, and the class rule. **Rates and medians for this step are not reported here; anyone who needs one must state population, window and as-of together.**
