...
open: a check that a new rule's selftest cases include one where only the new logic can produce the asserted answer — the case must fail when the rule is deleted; none exists.
