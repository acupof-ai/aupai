...
A third effect is a check that passed by coincidence rather than by construction. `review_present` FAILs a close 30 minutes after a close if no review row names it. #623 closed at 09:14Z and the approving row was written 08:12Z, inside the window, so the gate was satisfied -- by the timing, not by the chain. Had the close fallen an hour earlier the check would have FAILed naming a missing review, and the true cause would have been that the row had been discarded from `main`, not that no review happened. That is the shape rules 2 and 16 name from opposite sides: a green that does not express the property it is named for, and a retraction that never travelled.
...
Cost: one repo-wide commit freeze for the window between the sideways move and #625 landing, ~120 minutes (`runs/friction.jsonl`, `d8ef1711`). Three review rows absent from `main` for that window, during which #624 read as unreviewed. A fourth ref write escaped the guard entirely. Step timings from the run that carried the fix are not comparable to one another and must not be pooled: the step's own durations span 718-1024 s across its successful runs while the cancelled attempt shows 736 s, which measures the operator's cancellation threshold and not the step's cost (`ci.yml:73`'s `~2-4 min CPU` estimate for that step is stale by roughly 3x).
