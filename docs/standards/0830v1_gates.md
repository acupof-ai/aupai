...
- **A `broken()` proves one of a check's assertions fires, not all of them.**
  ...
  add the `data` and `runs` prefixes that were never scanned at all) then surfaced 32
  citations that had never been checked in the history of the repo. Rule: when a check
  asserts N things, its broken world must break each of them, or say in a comment which
  ones it does not cover.
- **A log file is not a source.** Of those 32, most cite `runs/*.log` — deleted, or
  overwritten by the next run of the same name. A fact whose source has rotted is a number
...
- **The attention fallback silently drops the document mask, and a frozen recipe key went
  ...
  **Landed 2026-08-31, and the assertion's premise changed with it.** The fallback now
  builds the block-diagonal causal mask from `cu` (`bucketize` over `cu[1:]`, `fcc6cb3`),
  so a missing package can no longer switch a frozen key off; it is checked against the
  per-document gold standard, against plain causal (the test can fail), and for
  cross-document leakage in `test_arch_compat.py`. It is a correctness floor, not a
  training path: fwd 624 ms / fwd+bwd 1451 ms at B=2, T=4096, H=8 bf16 on an H20, against
  0.5 ms warm for flash-attn 4 varlen at the same shape. The startup assertion therefore
  refuses `not HAS_FA` on a GPU for being ~20× slower, not for being wrong, and
