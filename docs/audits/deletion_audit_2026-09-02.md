...
**1a correction (c), 2026-09-02 (tilerl, sustained). Two rows of 1a are wrong, and both errors have one shape: the reader is a person waiting, not a file citing.** `5c2cb9c` deleted `scripts/rehearse_cursor.py`; it is restored byte-identical from `5c2cb9c~1` (sha256 `52d42096`) and given an entry-point row.
