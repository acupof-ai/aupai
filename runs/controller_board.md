...
## 2026-09-14 textbook vet (3b): exec 86% was classifier bug, code-only 96.4%
5089 src -> 4341 kept / 10.18M gate tokens. fail_class had counted sandbox RLIMIT_NPROC/thread/fork/third-party/network refusals as code (535/698 fails). Corrected: exec_pass_rate code-only 96.38% (A 96.10/B 96.49); exec_pass_rate_all_failures 86.16% kept for comparison. Real code fails 162 (Assertion 33, FileNotFound 21, coroutine-never-awaited ~20). decon 12, near_dup 3 (first), length 27, trunc 8. Handread unchanged: gen-A correctness 3.90 FAIL (only true blocker); gen-B 4.33 pass. New-prompt genA_0017 code-only 1.00. Production cache still blocked on gen-A handread, not exec.
