...
| DL-9 | S2 | **RESTATED — my original reading was wrong.** As published: `card_claim.claims()` reports a zombie as live, so its cards stay refused after the job ended. | The claim rested on `_is_zombie`'s docstring (card_claim.py:188, "callers that mean 'is the job running' must ask both") and on `claims()` not calling it. What I had not read: card_claim.py:566-574 states a zombie is deliberately NOT stale, and `acquire` **deletes every file `claims()` calls stale** (:425-428). | So filing a zombie as stale would delete a live claim and hand its cards to the next caller during a slow startup — 6e's ruling of 2026-09-03, reviewer e1. `_is_zombie`'s "ask both" caller is `status()` (:575), which reports, not `claims()`, whose answer is also a deletion. I attempted the fix, read the ruling, and reverted; the reasoning is now in `claims()`' docstring so the next reader does not repeat it. **The real defect is narrower and is fixed**: `check_card_held_without_claim` reported "no claim" for a zombie-HELD card and offered three causes (another tree, not claimed yet, another container) that all send the reader outside this tree while the claim sits in it. Its WARN now names the claim, the pid, the `Z` state and the release command, separately from the genuinely unclaimed cards. `card_claim.py --selftest` 49/49 after the revert. |
...
**Controller rulings received (6e, 2026-09-04, at `41061fcb` and `b1a4d344`):** DL-1 and DL-11
are post-audit design items and the first two on the fix queue when the audit closes — DL-1
with "cost is the variable, not the deadline", DL-11 as one transport for every `runs/*.jsonl`,
union by event. DL-22's fix is `--no-ff` in `merge_main.sh`. DL-21's fix absorbs de-45. Q2, Q3
and Q5 answered in-file after 44's pair check. **Area closed by the controller at `b1a4d344`**;
the `friction_review.md` review row goes to `runs/review.jsonl` at audit close.
