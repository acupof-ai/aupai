...
instrument was fixed. The three defects I already had fixes for are findings here, with the
unmerged commit named as evidence, not as a repair.
...
| DL-4 | S1 | `runs/launch_tests.json` records that `scripts/test_e2e.py` passed. | `scripts/test_e2e.py` at main before my unmerged `c8f9f25f`: `record_launch_test` sat at the end of the try body while stage 11 ran in the `finally`. b0 hit this at the Stage E shape. | A stage-11 `AssertionError` left the row reading `pass` for a run that exited nonzero. The row named no stage count, so a partial pass was unreadable as partial. Fix written, **not merged** (frozen): `c8f9f25f` moves the call into the `finally` after the last `stage()` and adds a `stages` field. |
| DL-5 | S1 | `test_e2e` stage 11 asserts that resuming from the run-end `.pt` refuses. | Since `c9011022` (de-31) the run-end save carries `step` and `opt` **on purpose**. | The assertion tests the opposite of the current contract and is stale; b0 was permitted to launch arm 2 on stages 1–10 with the failure recorded as stale. Inversion written, not merged (`c8f9f25f`): resume from the run-end save must SUCCEED, and a checkpoint with `step`/`opt` stripped must REFUSE. |
| DL-6 | S2 | `runs/launch_tests.json` certifies the arch tests for a launch. | One row per test path, last write wins. Stage E certifies two depths concurrently (L16 arm 1, L12 arm 2) and the L16 write erased the L12 rows. | `gate_arch_tests` cannot be green for both arms at once. Re-key on `(test path, shape)` written, not merged (`f2bd7bc0` on branch `de`). |
...
| DL-18 | S3 | The pod's `experiments.jsonl` shows what is running. | Pod: **0 rows with `status=running`**. Local: 3 (`b0_se_16lnew_1b` 02:28, `b0_se_looped_2b` 02:33 and 03:22). Container `ps`: only arm 1 (`b0_se_16lnew_1b`, torchrun 147080 + 2 ranks, etime 01:15). `runs/b0_se_looped_2b.log` ends in `SignalException: Process 195871 got signal: 15` and no `looped` process exists. `nvidia-smi`: cards 0,1 at 52 GB (arm 1), 2,3 at **0 MiB**, card 7 at 80 GB. | Arm 2 is dead, killed by SIGTERM, and both its rows still read `running` locally with no close. Cards 2+3 are free. Whether this is expected is b0's to say — I did not touch it (audit only). Not a ghost-row FAIL locally only because `no_ghost_running` is a pod-authority check. |
...
4. **DL-18**: arm 2 (`b0_se_looped_2b`) is dead by SIGTERM with two `running` rows open and
   cards 2+3 free. Whose call is the close, and is the kill expected?
...
_Pending: 44 recomputes three of the above and appends here._
