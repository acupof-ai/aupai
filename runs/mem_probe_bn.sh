...
#   CUDA_VISIBLE_DEVICES=4 setsid bash /work/aupai/runs/mem_probe_bn.sh \
#     > /work/aupai/runs/mem_probe_bn.wrap.log 2>&1 &
...
if [ -z "${CUDA_VISIBLE_DEVICES:-}" ]; then
  echo "refusing: mem_probe_bn needs CUDA_VISIBLE_DEVICES from the caller (cell bn is for card 4)." >&2
  exit 2
fi
...
export NGPU=1
export PORT=29514
exec python3 scripts/harness.py launch mem_probe_bn --training \
  --hypothesis "readout-4 probe cell bn on card 4: 300 steps, world 1, M1's launch line plus BatchNorm1d on the query output (Lample et al. 2019 3.3). Pass = pool_touched_frac at step 300 above 0.50 and not falling from step 200 to 300, read against mem_probe_base on card 0 (world 1) and NOT against M1's world-2 log." \
  -- ./run_ddp.sh --mix data/mix_200m_8b.json --name mem_probe_bn \
