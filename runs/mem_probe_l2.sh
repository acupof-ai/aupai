...
#   CUDA_VISIBLE_DEVICES=3 setsid bash /work/aupai/runs/mem_probe_l2.sh \
#     > /work/aupai/runs/mem_probe_l2.wrap.log 2>&1 &
...
if [ -z "${CUDA_VISIBLE_DEVICES:-}" ]; then
  echo "refusing: mem_probe_l2 needs CUDA_VISIBLE_DEVICES from the caller (cell l2 is for card 3)." >&2
  exit 2
fi
...
export NGPU=1
export PORT=29513
exec python3 scripts/harness.py launch mem_probe_l2 --training \
  --hypothesis "readout-4 probe cell l2 on card 3: 300 steps, world 1, M1's launch line plus per-half L2 on query and keys plus a learned temperature. Pass = pool_touched_frac at step 300 above 0.50 and not falling from step 200 to 300, read against mem_probe_base on card 0 (world 1) and NOT against M1's world-2 log." \
  -- ./run_ddp.sh --mix data/mix_200m_8b.json --name mem_probe_l2 \
