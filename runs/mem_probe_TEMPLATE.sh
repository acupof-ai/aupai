...
#   CUDA_VISIBLE_DEVICES=__CARD__ setsid bash /work/aupai/runs/mem_probe___CELL__.sh \
#     > /work/aupai/runs/mem_probe___CELL__.wrap.log 2>&1 &
...
if [ -z "${CUDA_VISIBLE_DEVICES:-}" ]; then
  echo "refusing: mem_probe___CELL__ needs CUDA_VISIBLE_DEVICES from the caller (cell __CELL__ is for card __CARD__)." >&2
  exit 2
fi
...
exec python3 scripts/harness.py launch mem_probe___CELL__ --training \
  --hypothesis "readout-4 probe cell __CELL__ on card __CARD__: 300 steps, world 1, M1's launch line plus __FLAGDESC__. Pass = pool_touched_frac at step 300 above 0.50 and not falling from step 200 to 300, read against mem_probe_base on card 0 (world 1) and NOT against M1's world-2 log." \
  -- ./run_ddp.sh --mix data/mix_200m_8b.json --name mem_probe___CELL__ \
