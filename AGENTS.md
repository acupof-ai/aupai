...
| `test_integration_tree_guard` | the tasks/friction/board writers refuse in the integration tree — the main worktree of a common git dir with linked worktrees, detached or not — and fail open where git cannot answer | the guard was widened or dropped; a refusal that fires in a `_tmp_repo()` world or a standalone clone breaks every fixture and CI, one that never fires puts rows back in the integration tree |
...
| Failures must be loud: checks before the write, raise or exit nonzero, never print-and-continue | 15 | §7 §13 §25 §51 §59 §136 §166 §181 §188 §193 §197 §204 §251 §256 §265 |
...
| A predicate set answers the question it enumerates, not the one it is named for; assert its population against the filesystem, never against a list; to measure a correction for an instrument, call the instrument; fix the criterion before widening the population | 5 | §275 §276 §277 §278 §289 |
...
- `harness task` and `harness friction` write the ledger of the tree they are invoked from: run them in your worktree, never in the integration tree, whose hook refuses EVERY commit there (2026-09-05 flip; no controller exemption). Enforced for all three ledgers — tasks, friction, board — at the WRITE rather than the commit, because by the time the hook refuses, the row is already dirty in the tree everyone merges through; that happened twice, ten minutes apart, before this existed. The predicate is `scripts/integration_tree.py`: main worktree AND has linked worktrees, both read from git, shared with the pre-commit hook so both read one implementation. Not the branch — the branch form answered `"HEAD"` once the tree was detached, going inert at exactly the moment the guard was needed. Not a path test — that would hardcode one laptop's layout and be wrong on the pod and in CI. The has-linked-worktrees clause keeps a standalone clone out: CI and all 27 of `harness.py`'s `git init` fixtures are their own main worktree and block nobody. It fails open where git cannot answer (no repository, git absent), because such a tree is not the integration tree and refusing there would break every `_tmp_repo()` fixture and every CI checkout; an unimportable predicate is loud instead of silent, since with the tree detached there is no branch test left to fall back to.
