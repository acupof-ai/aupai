...
| Provision a pod CONTAINER (not data) | `bash scripts/bootstrap_pod.sh [image\|deps\|cuda\|caches]` — image gate first (flash-attn 4 / `flash_attn.cute` is image-baked, never pip; fails loud if the base image lacks it), then the pinned training surface (`scripts/pod-training-direct.txt` versions constrained by `scripts/pod-constraints.txt`, torch +cu129 from the PyTorch index), a CUDA-build verify, and the HOST-only NVMe token-cache re-attach. Idempotent, stops on error. It does NOT fetch/build data or launch training — those live in `docs/standards/infra_persistent_rebuild_0916.md` (node/disk/mount) and `docs/standards/data_pipeline_rebuild_0916.md` (fetch→build→decontam) |
...
| Assemble the p1 keep-set domain | `python scripts/assemble_keep_p1.py` — hard-links the 685 audited keep-set shards into data/corpus/code\_keep\_p1 (zero bytes, source-prefixed names), writes build\_corpus\_stats.json, proves the mix guard accepts it |
...
| CED divergence watchdog | `python3 scripts/ced_diverge_watch.py --log runs/<name>.log --name <name>` (pod, started with the resumed run) — read-only; SIGTERMs the torchrun launcher by exact PID on gnorm>10 across 3 consecutive progress lines, 50-step mean train loss>3.0, or ANY card's `nvidia-smi` used RSS >80 GiB (`--mem_thresh_gib`, sampled every `--mem_interval` 60s; worst-case detection latency ~60s, slower than the log rules); reports, never restarts (`runs/prereg.jsonl#v41_ced_0923` amendment_2 gnorm/loss, amendment_3 RSS rule 2). The memory basis is `nvidia-smi memory.used` (RSS, ~72 GiB on this stack), NOT the train log's `torch.cuda.max_memory_allocated` (~43 GiB, allocator-only, cannot reach the limit). Exit 3 on a read/parse fault (a shaped-but-unparsable log line OR an nvidia-smi read/number failure is blindness, not health), 4 if the log goes stale, 5 if a trigger fired but no launcher matched. Known-answer: `--selftest` (pure + fake-smi live), the pod `--once` kata over the real seg1 log (12630–12700 fires, 12000–12600 silent), and live fake-smi RSS over/under/read-fault |
...
| `ledger append --path runs/X.jsonl --row '<json>'` | the guarded writer for hand-maintained ledgers (retro, ledger_resolutions, any future union ledger); refuses outside `runs/*.jsonl` and in the integration tree |
| `review add --reviewer … --pr … --task … --artifact … --verdict …` | the guarded writer for `runs/review.jsonl`; `--owner` is optional and never checked against `--reviewer` — reviewing a PR that is not yours is the normal case |
...
| `test_integration_tree_guard` | every session ledger writer refuses in the integration tree — the main worktree of a common git dir with linked worktrees, detached or not — fails open where git cannot answer, and W11/W12 enumerate the writer population from `git ls-files` and the union ledgers from `.gitattributes` at test time | the guard was widened or dropped; a refusal that fires in a `_tmp_repo()` world or a standalone clone breaks every fixture and CI, one that never fires puts rows back in the integration tree, a new unguarded writer or a new union ledger outside `runs/*.jsonl` must turn W11/W12 red by itself |
...
The consequence is not that ChatML is under-taught. `eval/code_zh.py` and `eval/math_zh.py` prompt through `format_prompt`, so **every generative number ever taken on a base checkpoint handed the model a prefix that appears nowhere in its training data**, then scored the continuation — the model repeats the input or drifts to web boilerplate, which is what an unseen prefix produces. How large the format effect is, from the fact store: `facts/base_eval.json#be.l1_fewshot_p324` measures answer-present on ckpt_p324, math, continuation format, n=497, at 0/1/3 demos — **25.4% / 53.5% / 63.6%, +38.2pt across the sweep with the model held fixed**. Format exposure alone moves this field by 38 points, which is the size of the effect an unseen prefix can produce. (A `94.4% def ` vs `0.3% under ChatML` pair circulated here until 2026-09-03 and was REMOVED: it appears in no `facts/*.json` entry, and its only other mention, `docs/lessons/honest_measurement_prereg.md:103`, is a passing reference with no checkpoint, no n and no artifact path. The rule below never depended on it; e1 caught it being cited as evidence by fb.) **Base generative zeros taken before this date measure response to an unseen prefix, not capability.** Rules that follow: base evals prompt in continuation format, which is in-distribution; a base eval may not introduce a token sequence absent from pretraining; SFT does teach ChatML from nothing, so the SFT-side loss-mask check below is unaffected. `scripts/test_sft_pack.py` checks the loss mask directly (CI): every masked span ends at `assistant\n`, the turn terminator is supervised.
...
- **`tn exec` and `~/bin/pod` are two different filesystem views with the same hostname, and the wrong view does not error — it answers.** `tn exec` runs on the HOST (where `crictl` lives); `~/bin/pod` runs INSIDE the container, landing in `/sgl-workspace/sglang`, so a command must `cd /work/aupai` itself. Read and write container artifacts — code, logs, checkpoints, corpus — with `~/bin/pod` only; use `tn exec` for host-level queries (`nvidia-smi`, `ps`, `crictl`), which agree on both sides because GPU and process state are machine-wide. That agreement is why the confusion survived for days. The tell that you are on the host: `python3 -V` is 3.7.3, `import torch` raises ModuleNotFoundError, and `/work/aupai` is a stale tree with nothing newer than 2026-09-01. The container is python 3.12.3, torch 2.11.0+cu129, 8 devices. `ls`/`tail`/`grep` all "work" and return the wrong decade of the file.
  - From the host the container's live volume is readable at `/var/lib/kubelet/pods/<id>/volumes/kubernetes.io~empty-dir/work` (discover `<id>` as `scripts/pod_push.sh:71` does) — real data, still the host's interpreter, so torch and `/proc/<container pid>` stay absent.
  - Measured 2026-09-03: this bullet already said "with `~/bin/pod` only" and was violated anyway, because a host-view `grep` agreed with a second wrong reading — `git show <pod_synced_head sha>:eval/domain_loss.py`, which is what was PUSHED at stamp time, not what is on the pod now. **Two wrong views agreeing is what made it convincing.** Verify pod file contents in the container, never from a sha on the laptop.
- **A PID is only meaningful in the namespace that read it; GPU UUID plus cmdline are the only cross-boundary identities.** The two views number the same process differently — a rank is 1738493 on the host and 1382917 in the container, neither wrong, neither resolving in the other. So "kill by exact PID" means *by a PID read in the namespace you are about to kill from*, and handing a process to another session names it by `nvidia-smi --query-compute-apps=pid,gpu_uuid,used_memory` plus cmdline, both machine-wide. Two failures on 2026-09-01, in opposite directions: a job queued behind `while [ -d /proc/1302052 ]` used a HOST pid inside the CONTAINER, so the guard was false on its first evaluation and the job landed on cards a probe held, contending ~80 steps; and two sessions quoted each other pid sets neither could look up. **Never guard a launch on `[ -d /proc/<pid> ]`**, and not only across the boundary: a ZOMBIE keeps its `/proc` entry until reaped, so the test stays true for a process that has already exited — fb queued a scoring chain behind `[ -d /proc/3739057 ]` in the SAME namespace on 2026-09-03, the eval finished leaving `3739057 Zs [bash] <defunct>`, and the queue waited 31 minutes on a dead pid with card 6 idle. `/proc` answers "has this pid been reaped", never "is this job running"; poll a log string instead, which is namespace- and reap-independent. And a `for` loop is NOT the queue: `harness launch` returns as soon as it has launched, so the next iteration fires immediately and the lane guard refuses it — measured minutes after the zombie fix, when `bench_step7500_score_matrix` was refused with `lane GPU 6 occupied by pid 1786944`, which was step7000 launched one second earlier. The guard working; the advice still wrong.
- **A dropped tn tunnel does not end the command it started.** `tn exec` returns its own exit code when the connection drops; the `crictl exec` chain keeps running in the container. On 2026-09-03 a `cp -a /work/aupai/. /tmp/absmoke/` (206 GB of `data/`) filled the pod to 100%, and the `rm -rf` that followed lost ground for ten more minutes because the `cp` was still going — the operator had read tn's `[exited with code 1]` as "the command finished". After any long command whose tunnel dropped, go back and check by pid in the container; a local exit code is not a statement about the remote process. This is the mirror of the `setsid` rule below: that one keeps a job alive on purpose, this one is a job that stayed alive by accident.
...
**The two redirects are at different scopes and flattening them silences the log.** `> runs/x.log 2>&1` is INSIDE the `bash -c '...'` quotes and belongs to the command; `>/dev/null 2>&1` is outside and belongs to `setsid` itself. Drop the `bash -c` wrapper and both land at one level, where redirection applies left to right and the last one wins — so `/dev/null` takes the output and `runs/x.log` stays 0 bytes. The job runs, the card is busy, `ps` is healthy, and every progress line is gone; artifacts the script writes itself survive, so it looks fine until someone needs the log (e1, 2026-09-03, caught in 45 s by `ls -la` on the log, 1 min to relaunch). **After any detached launch, `ls -la` the log before walking away** — three signals say the job is alive and only the log says it is mute.
...
- **File transfer into the container: `podput <local> <remote-abs-path>`** (gzip+base64, 100KB cap — the real limit is the remote argv, not a constant). **The second argument is not validated: a relative path resolves against the container's default cwd, which is another project's source tree** (`podput model.py model.py` dropped four files into `/sgl-workspace/sglang/`, tilerl 2026-09-03; nothing tracked was overwritten and they were removed by exact path). `tn push` lands on the HOST filesystem and is invisible inside the container — never use it to deliver code or data to the repo.
...
- **`data/pod_head_manifest.txt` is NOT tracked. `scripts/pod_push.sh` generates it from the HEAD it ships, and ships it LAST** (shape A, 2026-09-04). Nobody runs `pod_drift.py --write` by hand and no commit carries the manifest. The order is the safety: a push that dies between the files and the manifest leaves the pod holding the OLD manifest, which `--check` correctly calls drift, where the other order would leave a NEW manifest vouching for files that never landed. Tracking it was the largest single friction cause in `runs/friction.jsonl` (4 of 15 rows) and two mechanisms were measured: a path-scoped commit cannot stage it, because `git commit -- <paths>` builds a temporary index the hook's `git add` writes to and git discards; and a MERGE commit cannot carry it at all, because `pre-merge-commit` runs after git has built the tree, so the regen only ever reaches the index — permanently dirty against HEAD, aborting the next merge. It also produced a real defect: at `81f091af` a clean textual merge spliced one side's manifest line with the other side's file content, so main asserted `d07a474f` for a roadmap whose tree held `8dc68958` (1 stale of 423). `--check-head` is gone with it: "does the committed manifest describe HEAD" has no subject once the manifest is generated from HEAD, and running the flag now refuses loudly rather than exiting 0.
...
Rules and their enforcing checks live in `docs/lessons/gate_failure_shapes.md`; incidents live in `docs/lessons/gate_failure_incidents.md`. 33 closed incidents were deleted 2026-09-04 (see commit). **The surviving count is not written here** — `shapes_table_covers_doc` verifies each table row against the doc and never reads this prose, so a number here drifts unseen: 164 against a table of 165 before 2026-09-05, then 223 against 237 on 2026-09-08, where the September fix was to write a corrected number and it drifted again in three days. The count is whatever `harness check` prints.

| Rule | Shapes | §refs |
|---|---|---|
| Verify premises before acting, sources before citing; a correct conclusion does not certify its argument | 24 | §8 §14 §18 §37 §38 §46 §49 §52 §57 §66 §70 §96 §106 §131 §139 §175 §179 §190 §198 §199 §211 §246 §263 §271 |
| A criterion must express the property asked; test it on known-answer positive and negative worlds before trusting output | 132 | §9 §10 §23 §26 §29 §31 §34 §35 §40 §45 §48 §54 §56 §61 §65 §67 §69 §71 §72 §73 §75 §76 §77 §80 §81 §84 §85 §89 §90 §91 §94 §97 §98 §103 §108 §110 §112 §114 §121 §125 §128 §132 §134 §135 §137 §140 §141 §142 §146 §147 §148 §149 §150 §151 §153 §158 §165 §169 §170 §171 §173 §174 §176 §177 §178 §180 §183 §184 §186 §187 §191 §196 §200 §201 §202 §203 §205 §206 §207 §208 §209 §212 §213 §215 §216 §217 §218 §219 §220 §221 §222 §223 §224 §225 §226 §227 §228 §229 §231 §232 §233 §234 §235 §236 §237 §238 §239 §240 §241 §242 §243 §244 §247 §248 §249 §250 §252 §253 §254 §257 §258 §259 §266 §267 §268 §270 §272 §273 §274 §283 §287 §300 |
| Artifacts carry their producer's identity; missing identity refuses, never rebuilds | 8 | §4 §24 §44 §182 §189 §210 §284 §292 |
| Failures must be loud: checks before the write, raise or exit nonzero, never print-and-continue | 17 | §7 §13 §25 §51 §59 §136 §166 §181 §188 §193 §197 §204 §251 §256 §265 §288 §291 |
| State the vision before the number; outside it, label unmeasured, not absent | 11 | §3 §5 §6 §17 §19 §28 §30 §32 §36 §53 §100 |
| Every number carries its basis: source type, resolution, algorithm; label extrapolation | 39 | §1 §11 §12 §20 §21 §50 §55 §62 §63 §64 §79 §86 §99 §104 §105 §109 §111 §115 §117 §118 §124 §127 §133 §143 §152 §155 §156 §157 §159 §161 §164 §172 §185 §192 §230 §260 §269 §285 §286 |
...
| A hardlink is the same file; a staged "copy" you then write in place truncates the source inode silently — copy or temp+rename for any corpus bytes you will mutate | 1 | §302 |
...
| To hold the tree quiet, `scripts/merge_main.sh --hold`, | manual: which command took the lock is not recoverable from the lock. A hand-rolled `mkdir` and a crash between `--hold`'s mkdir and its write leave the same holderless directory, which is why the 3-second grace exists at all; a check that refused every holderless lock would refuse the crash case forever, and one that read the holder file cannot see a lock taken without one. `merge_main.sh --selftest` asserts the grace in both directions, which is the mechanism, not the discipline |
...
How many rules, and how many are checked: `harness check`'s `agents_rules_covered` line
prints it. No number here. Writing one has failed three times — stale at "35 rules: 14
checked, 21 manual" against the code's 36/13/23, stale again at "39 rules: 18 checked, 21
manual" on 2026-09-05, and stale a third time at "56 rules: 23 checked, 33 manual" while
this very paragraph said the count was regenerated rather than written. A sentence that
states the rule and then breaks it in its own first clause is the drift generator, not the
number.
...
**Fact store — `facts/*.json`.** Measurements live here, one file per domain, never in prose. Required per entry: `id`, `value`, `measured` (YYYY-MM-DD), `source` (command or artifact), `config` (non-empty), `uncertainty`, `status` (`measured` / `recorded` / `unmeasured` / `retracted`). `unmeasured` and `retracted` also need `claim`, `audit`, `refuted_by`. Optional: `unit`, `guard_phrases` (must not reappear in AGENTS.md), `boundary` (what the measurement cannot answer). `facts_well_formed` enforces the required fields.

A `retracted` entry also needs **`retracted_value`: the list of values that are WRONG, as data.** `value` cannot answer that question — a retraction rewrites it into a narration, so one field ends up holding both the dead number and the number that replaced it, separated only by prose. **`[]` is a real answer and the common one:** of the nine retracted entries read 2026-09-04, five retract a conclusion while their numbers stand — `ds.second_resume_rereads_one_segment`'s 8,192 rows was true of every checkpoint before `52aec31`, and marking that dead would be the same defect in a new place. So a tool hunting retracted numbers reads this field and never `value`. The field is hand-maintained and rots when someone edits `value` and leaves the list behind, which is why `facts_well_formed` also checks every listed value still appears in the entry's `value` or `claim`.

The domain files, named here because `facts_well_formed` requires it in both directions — every `facts/*.json` must appear in AGENTS.md and every name here must exist, so a new fact file is red until it is listed and a deleted one is red until it is not: `facts/base_eval.json`, `facts/contamination.json`, `facts/corpus_filters_baseline.json`, `facts/corpus_supply.json`, `facts/data_quality.json`, `facts/data_scaling.json`, `facts/deepseek_v4.json`, `facts/distillation.json`, `facts/efficiency.json`, `facts/memory_layers.json`, `facts/minicpm5_arch.json`, `facts/moe.json`, `facts/multilingual.json`, `facts/smelt_deeploop.json`, `facts/source_baseline.json`, `facts/tokenizer.json`, `facts/v41.json`. What each holds is in the file; read the ids rather than a description here, which is the half nothing checks.

**Coordination.** Several sessions work on this repository. Announce before editing `train.py`/`sft*.py`/`AGENTS.md`, commit promptly, hand the file back. Commit messages in English, one concern per commit. **The subject ends with the committing session's roster name in parentheses** (user ruling 2026-09-07), e.g. `merge_main: walk the range (de)`: every session commits as one git author, a merge erases branch-based attribution, and the user chose a subject marker over per-worktree `user.email`. This is the project override of the global "no attribution trailers" rule for this one token; tool and session-link trailers stay banned.
...
- `harness task` and `harness friction` write the ledger of the tree they are invoked from: run them in your worktree, never in the integration tree, whose hook refuses EVERY commit there (2026-09-05 flip; no controller exemption). Enforced at the WRITE rather than the commit for every session ledger writer — tasks, friction, board, review (`harness review add`), milestones, msg_log, prereg, experiments, policy_metrics, sweeper, and the writerless ledgers retro and ledger_resolutions through the generic `harness ledger append --path runs/X.jsonl --row '<json>'` — because by the time the hook refuses, the row is already dirty in the tree everyone merges through; that happened twice, ten minutes apart, before this existed. The population is enumerated from the filesystem, never a list: `test_integration_tree_guard` W11 scans every tracked writer and W12 reads the union ledgers from `.gitattributes`, so a new unguarded writer or a new union ledger turns the test red by itself (de-98; the measured population was 3-of-10 guarded when the enumeration landed, against the 1-of-4 estimate the task replaced). Run-side writers (eval/, datagen/, train-side diag) are adjudicated exclusions in the test, each with a reason; the pod is not a git repo, so the guard fails open there. The predicate is `scripts/integration_tree.py`: main worktree AND has linked worktrees, both read from git, shared with the pre-commit hook so both read one implementation. Not the branch — the branch form answered `"HEAD"` once the tree was detached, going inert at exactly the moment the guard was needed. Not a path test — that would hardcode one laptop's layout and be wrong on the pod and in CI. The has-linked-worktrees clause keeps a standalone clone out: CI and all 27 of `harness.py`'s `git init` fixtures are their own main worktree and block nobody. It fails open where git cannot answer (no repository, git absent), because such a tree is not the integration tree and refusing there would break every `_tmp_repo()` fixture and every CI checkout; an unimportable predicate is loud instead of silent, since with the tree detached there is no branch test left to fall back to.
...
- **A conflicting path needs a commit first, and read which path it is.** `git merge` names the file in its `would be overwritten` line. A derived artifact is nobody's work: `git checkout -- <path>`, then merge again. (`data/pod_head_manifest.txt` was the example here and is no longer tracked, so it can no longer collide.) A file you wrote gets a path-limited `git commit <paths> -m "wip: ..."` first. A `wip:` commit is yours on your branch; a stash is everyone's. `AUPAI_BEHIND_MAIN_OK=1` is not the way out of either: it commits onto code main has moved past.
- `runs/*.jsonl` ledgers merge by union (`.gitattributes`); row identity is `(name, started)` / `id`, never position. `EXPERIMENTS.md` is rendered from that union-merged ledger and DOES conflict — two branches each appending a row and rendering collide on the newest-first top lines, measured 2026-09-04 — so it carries a merge driver that regenerates it. The driver only fires on a conflict, which is why `data/pod_head_manifest.txt` never benefited from having one: two branches touching different manifest lines merge cleanly and git splices them.
