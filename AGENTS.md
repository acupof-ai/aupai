...
- `harness task` and `harness friction` write the ledger of the tree they are invoked from: run them in your worktree, never in the integration tree (b0, 2026-09-04). **Enforced from 2026-09-05, and the scope is all three ledgers — tasks, friction, and board.** The predicate is `scripts/integration_tree.py`'s `is_integration_tree`, shared with the pre-commit hook so both read one implementation: **the main worktree of a common git dir that has linked worktrees.** Not the branch — that was the first version and it went inert within hours, when the integration tree was detached on purpose (main `0425accb`) and `symbolic-ref` began answering `HEAD`. Not a path either: `/Users/bytedance/code/aupai` is one laptop's layout and is wrong on the pod and in CI. The has-linked-worktrees clause is what keeps a standalone clone out — CI and all 27 of `harness.py`'s `git init` fixtures are their own main worktree and block nobody. It fails open where git cannot answer (no repository, git absent), because such a tree is not the integration tree; an unimportable predicate is loud instead, since with the tree detached there is no branch test left to fall back to. There is no `AUPAI_CONTROLLER` lift: it existed only while `merge_main.sh` appended a friction row from the integration tree, and the new one writes from the caller's worktree. The refusal is at the write rather than the commit because by the time the hook refuses, the row is already dirty in the tree everyone merges through — which is what happened twice, ten minutes apart, before this existed.
