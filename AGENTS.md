...
- **`tn exec` and `~/bin/pod` are two different filesystem views with the same hostname.** `tn exec` runs on the host (where `crictl` lives); `~/bin/pod` runs inside the container. A file present in one view is not necessarily present in the other. Read and write container artifacts — code, logs, checkpoints, corpus — with `~/bin/pod` only. Use `tn exec` for host-level queries (`nvidia-smi`, `ps`, `crictl`). GPU and process state are machine-wide, so those queries agree on both sides — that is why the confusion survived for days. **The tell that you are in the host view: `python3 -V` is 3.7.3, `import torch` raises ModuleNotFoundError, and `/work/aupai` is a stale tree with no `runs/` and nothing newer than 2026-09-01.** The container is python 3.12.3 with torch 2.11.0+cu129 and 8 devices. The wrong view does not error — it answers, with a world that looks stale rather than absent, so `ls`/`tail`/`grep` all "work" and return the wrong decade of the file. `~/bin/pod` lands in `/sgl-workspace/sglang`, so a command must `cd /work/aupai` itself. From the host the container's live volume is readable at `/var/lib/kubelet/pods/<id>/volumes/kubernetes.io~empty-dir/work` (discover `<id>` as `scripts/pod_push.sh:71` does) — real data, but still the host's interpreter, so torch and `/proc/<container pid>` stay absent. Measured 2026-09-03: this bullet already said "with `~/bin/pod` only" and was violated anyway, because a host-view `grep` agreed with a second wrong reading — `git show <pod_synced_head sha>:eval/domain_loss.py`, which is what was PUSHED at stamp time, not what is on the pod now. Two wrong views agreeing is what made it convincing. Verify pod file contents in the container, and never from a sha on the laptop.
- **A PID is only meaningful in the namespace that read it. GPU UUID and cmdline are the only cross-boundary identities.** The host and the container number the same process differently: `tn exec` sees a rank as 1738493 while `~/bin/pod` sees it as 1382917. Neither is wrong and neither resolves in the other view. Two failures from this on 2026-09-01, in opposite directions: a job queued behind `while [ -d /proc/1302052 ]` used a *host* pid inside the *container*, where it does not exist — the guard was false on its first evaluation, so the job launched immediately onto cards a running probe held and contended with it for ~80 steps; and separately two sessions quoted pid sets to each other that neither could look up. So: "kill by exact PID" implicitly means "by a PID read in the namespace you are about to kill from" — read and kill in the same view. To hand a process to another session, or to guard on one, name it by GPU UUID (`nvidia-smi --query-compute-apps=pid,gpu_uuid,used_memory`) plus its cmdline, both of which are machine-wide. **Never guard a launch on `[ -d /proc/<pid> ]`** — and not only across the boundary. A ZOMBIE keeps its `/proc` entry until it is reaped, so the test stays true for a process that has already exited: fb queued a scoring chain behind `[ -d /proc/3739057 ]` in the SAME namespace on 2026-09-03, the eval finished and left `3739057 Zs [bash] <defunct>`, and the queue waited 31 minutes on a dead pid with card 6 idle. The namespace half of this rule made the same-namespace case read as safe, which is why it was written; `/proc` answers "has this pid been reaped", never "is this job running". Poll a log string instead — namespace-independent AND reap-independent. **A `for` loop is NOT the queue**: `harness launch` returns as soon as it has launched, so the next iteration fires immediately and the lane guard refuses it — measured minutes after the zombie fix, when `bench_step7500_score_matrix` was refused with `lane GPU 6 occupied by pid 1786944`, which was step7000, launched by the previous iteration one second earlier. The lane guard caught it and wrote no ledger row, which is the guard working; the advice was still wrong. Sequencing two detached jobs needs the log-string poll, not the absence of a guard.
...
| Every number carries its basis: source type, resolution, algorithm; label extrapolation | 35 | §1 §11 §12 §20 §21 §50 §55 §62 §63 §64 §79 §86 §99 §104 §105 §109 §111 §115 §117 §118 §124 §127 §133 §143 §152 §155 §156 §157 §159 §161 §164 §172 §185 §192 §230 |
...
| Shared resources are explicitly exclusive; co-residency is judged by each implementation's measured cost in seconds against the run's own spend, never by metric class. A claim says who intends to hold a resource and nothing about whether the job is progressing | 6 | §15 §126 §194 §195 §214 §245 |
...
| A failed push means your advance may already be gone | manual: the omission is a person not running one command after a failure; the repo records the forced ref, never the unverified belief |
...
**Fact store — `facts/*.json`.** Measurements live here, one file per domain, never in prose. Required per entry: `id`, `value`, `measured` (YYYY-MM-DD), `source` (command or artifact), `config` (non-empty), `uncertainty`, `status` (`measured` / `recorded` / `unmeasured` / `retracted`). `unmeasured` and `retracted` entries also need `claim`, `audit`, `refuted_by`. A `retracted` entry also needs `retracted_value`: the list of values that are WRONG, as data. It exists because `value` cannot answer that question — a retraction rewrites `value` into a narration, so one field ends up holding the dead number and the number that replaced it, separated only by prose (`eff.depth_is_not_the_mfu_gap` carries retracted 14.2/16.0 beside correct 43.5/23.7; every number in `eff.dynamo_recompile_not_a_lever` comes from the measurement that superseded it). **`[]` is a real answer and the common one:** read all nine retracted entries 2026-09-04 and five retract a conclusion while their numbers stand — `be.l1_3shot_retracted`'s rerun reproduced its 0.2/63.6/8.9, `cont.sft_all_code_holdout_leak`'s v2 2.2% and v3 40.0% are restored, `ds.second_resume_rereads_one_segment`'s 8,192 rows was true of every checkpoint before `52aec31`. Marking those dead would be the same defect in a new place, so a tool that hunts retracted numbers reads this field and never `value`. Optional: `unit`, `guard_phrases` (must not reappear in AGENTS.md), `boundary` (what the measurement cannot answer). `facts_well_formed` enforces the required fields, and for `retracted_value` also that every listed value appears in the entry's `value` or `claim` — the field is hand-maintained, so it rots when someone edits `value` and leaves the list behind. Domain files: `facts/tokenizer.json` (fingerprint, sizes, gate values, frontier, sweeps), `facts/contamination.json` (math holdout containment), `facts/data_scaling.json` (scaling-law and token-budget measurements), `facts/multilingual.json` (corpus stats, fertility, zh:en ratio/transfer evidence, token supply), `facts/data_quality.json` (hand-read quality audits, filter retention, domain loss drops, quality-head AUC), `facts/base_eval.json` (base-model eval resolution at 200M, minimal-pair construction rules, sample-size math, MC language confound), `facts/efficiency.json` (hardware specs, throughput, hybrid-ratio and AttnRes evidence), `facts/corpus_supply.json` (measured landed tokens per 30b domain vs the mix_30b `_blocked` targets, each with its measurement basis), `facts/smelt_deeploop.json` (SMELT/DeepLoop paper-reported numbers, derived AttnRes loop costs, TPP comparison), `facts/memory_layers.json` (sparse product-key memory: what a 1.07B-parameter reachable table bought on in-training val, the key-usage collapse that stopped all three arms, throughput cost per pool size, and the table's real bytes/param — every entry is from a STOPPED arm and says so in its own `boundary`), `facts/moe.json` (MoE load-balancing constants: DeepSeek-V3's bias update speed γ=0.001 and sequence-wise balance α=1e-4, both `recorded` paper-reported values from arXiv:2412.19437 §2.1.2/§4.2 — read, not relayed — each carrying in its `boundary` the fact that their regime is 1,966,080 tokens per routed expert per step against our 32,768, a factor of 60.0, so the constants are borrowed starting points and not inherited results).
...
- **Never write `refs/heads/main` by hand.** No `git update-ref refs/heads/main`, no `git branch -f main`, no `git push --force`. A bare `update-ref` takes no expected-old-value, so it moves the ref unconditionally and discards whatever landed since you read it — measured 2026-09-06: main went `bc95abe8 -> 9a11b9ea` sideways and a task-close commit left the history with nothing red, `merge_main` having printed a successful advance minutes earlier. **A refused push is fixed by amending on your BRANCH and re-running `merge_main`**, which is the case that produced this one: GitHub's push protection rejected a commit for a credential, and the redacted amend was forced onto `main` instead of merged. The CAS would have refused it; it was bypassed rather than defeated. `harness check main_advances_by_ancestry` reads main's reflog and FAILs on any move whose previous value is not an ancestor of the new one.
- **A failed push means your advance may already be gone.** `merge_main` printing `main X -> Y` reports that the CAS returned 0, not that Y survived. If the push fails, verify before doing anything else: `git merge-base --is-ancestor <your-sha> main`. Re-landing is a normal merge from your branch — but check first that your branch's ancestry does not carry whatever the push refused, or the second attempt fails the same way.
