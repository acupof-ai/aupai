...
| Decontaminate gate domains against the code evals | `python scripts/filter_gate_domains.py --domains <comma-list>` — 13-gram overlap removal against HumanEval/MBPP, writes a `_dc` domain + summary; a gate prerequisite for every UltraData code domain (ae, facts in `facts/contamination.json`). The overlap engine is the library `filters/decontam_ngram.py` (CLI runs only with `--selftest`); the runnable command is this script |
