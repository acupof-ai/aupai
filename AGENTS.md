...
| CED divergence watchdog | `python3 scripts/ced_diverge_watch.py --log runs/<name>.log --name <name>` (pod, started with the resumed run) — read-only tail; SIGTERMs the torchrun launcher by exact PID on gnorm>10 across 3 consecutive progress lines or 50-step mean train loss>3.0, reports, never restarts (`runs/prereg.jsonl#v41_ced_0923` amendment_2). Exit 3 on a read/parse fault (a shaped-but-unparsable line is blindness, not health), 4 if the log goes stale. Known-answer: `--selftest` (pure layer) and the pod `--once` kata over the real seg1 log (12630–12700 fires, 12000–12600 silent) |
