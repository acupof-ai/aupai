...
Rules and their enforcing checks live in `docs/lessons/gate_failure_shapes.md`; incidents live in `docs/lessons/gate_failure_incidents.md`. 33 closed incidents were deleted 2026-09-04 (see commit). **The surviving count is not written here.** This sentence has drifted twice for the reason it already stated: 164 against a table of 165 before 2026-09-05, then 223 against 237 on 2026-09-08, because `shapes_table_covers_doc` verifies each table row against the doc and never reads this prose. The fix in September was to write a corrected number, which drifted again in three days. The count is whatever `harness check` prints.
