...
for _ in $(seq 1 120); do
  if _take_lock "merge $1 into main" no; then
    ...
    #
    # DRAINED AFTER THE CAS AND TRUNCATED ONLY AFTER THE COMMIT SUCCEEDS, so a kill anywhere in
    # between leaves the queue intact and the rows land next time. The failure this ordering
    # rules out is losing a row, which is the only irreversible outcome here: a row committed
    # twice would be visible and fixable, a row dropped is gone.
