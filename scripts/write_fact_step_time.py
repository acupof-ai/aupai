...
def main():
    ...
    fact = {
        ...
        "uncertainty": (
            ...
            f"it -- {table[0][2]:.4f} over all lines against {fast_med:.4f} over the "
            f"uncontended ones -- and the fastest decile of EACH arm cannot contain a step that "
            f"was waiting on host I/O, which is why the band's lower end "
            f"({lo:.4f}) is the confound-free estimate and its upper end ({hi:.4f}) the "
            f"as-observed one. Single seed (42) per side, so no seed spread exists and none is "
