...
"""Merge drivers for the two DERIVED files that blocked six merges on 2026-09-03.

Both are regenerated from a source of truth, so a textual conflict in them is noise: the
resolution is always "regenerate", never "pick a side". A driver that regenerates is the
root fix the friction ledger's top cause asks for.

    data/pod_head_manifest.txt   regenerated by scripts/pod_drift.py --write-index
...
Usage:
    ...
    python3 scripts/merge_drivers.py --selftest    # a real two-branch conflict, both files
    python3 scripts/merge_drivers.py regen-manifest %A %O %B %P
...
DRIVERS = {
    "aupai-manifest": {
        "path": "data/pod_head_manifest.txt",
        "cmd": "python3 scripts/merge_drivers.py regen-manifest %A %O %B %P",
        "regen": ["python3", "scripts/pod_drift.py", "--write-index"],
        "why": "regenerated by the pre-commit hook on every commit, so any two branches that both commit conflict",
    },
    "aupai-experiments": {
        "path": "EXPERIMENTS.md",
...
def main():
    ...
    if a[0] == "--selftest":
        selftest()
        return 0
    if a[0] == "regen-manifest":
        return regen("aupai-manifest", a[1:])
